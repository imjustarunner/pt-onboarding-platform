/**
 * Standard popular race distances used across the Summit Stats Race Divisions system.
 *
 * minMiles / maxMiles define non-overlapping ranges for matching a tagged race workout
 * to a specific division.  A workout must have is_race = 1 AND its distance_value must
 * fall within the range to count.
 *
 * defaultEmoji is the fallback icon when no custom emoji has been set at the club level.
 */
...
/** All keys enabled by default when no per-season config exists. */
