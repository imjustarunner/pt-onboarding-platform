...
export function interviewCalendar({ startsAt, endsAt, timezone = 'America/Denver', title, publicJoinUrl, interviewId = 'preview', description: customDescription = null, uid = null, now = new Date() }) {
  ...
  const description = customDescription || `We look forward to meeting you. Join your interview: ${publicJoinUrl}`;
  ...
  const ics = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//People Operations//Interview//EN', 'CALSCALE:GREGORIAN', 'METHOD:PUBLISH', 'BEGIN:VEVENT',
    `UID:${escape(uid || `hiring-interview-${interviewId}@people-operations`)}`, `DTSTAMP:${stamp(now)}`, `DTSTART:${stamp(start)}`, `DTEND:${stamp(end)}`,
