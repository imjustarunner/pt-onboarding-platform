...
export function supervisionEmailBody({session,recipientName,hostNames,people,calendar,joinUrl,rsvpUrl,presentationUrl,detailsUrl,isRequired,isPresenter,isHost,kind='invitation'}) {
 const group=String(session.session_type).toLowerCase()==='group';
 const title=group?'Group supervision':'Individual supervision';
 ...
 const status=isHost?'You’re hosting this session':isPresenter?'You’re presenting':signup?'Please RSVP to confirm your place':isRequired?'Your attendance is required':group?'Optional session':'Your supervision appointment';
 const intro=isPresenter?'Thank you for presenting your case conceptualization. Review your presentation and update it before we meet.':group?'Join us for case discussion, shared perspectives, and supportive professional growth.':'A dedicated time to connect with your supervisor, ask questions, and continue your professional growth.';
 ...
 ${row('Date',calendar.dateLabel)}${row('Time',calendar.timeLabel)}${row('Time zone',session.event_timezone||'America/Denver')}${row('Format',inPerson?session.location_text||'In person':'Virtual supervision')}${row(group?'Host':'Supervisor',hostNames.join(', '))}${presenters.length?row('Case conceptualization presenter',presenters.join(', ')):''}</table>
 ${session.recurrence_series_id?'<p style="color:#53655e;">This is part of a recurring supervision series. This reminder and your RSVP apply to the date shown above. See your schedule for all dates.</p>':''}
 ${button(inPerson?'View session details':group?'Join Group Supervision':'Join Supervision',inPerson?detailsUrl:joinUrl,true)}
 ...
 <p style="font-size:13px;color:#63736b;">You can also open the attached supervision.ics file in your calendar app.</p>${button('View Meeting Details',detailsUrl)}
