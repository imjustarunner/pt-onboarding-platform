...
export {
  buildParentingConfidenceSummary,
  buildParentingConfidenceSummary as buildSummary,
  domainsForContext,
  calculateParentingConfidenceIndex,
  calculateImportanceWeightedParentingIndex,
  calculateParentingOpportunityScore,
...
function mapDomain(d) {
  return {
    ...
    parentingSystem: d.parenting_system,
...
export async function getDefaultTemplate({ agencyId = null } = {}) {
  ...
  if (agencyId) {
    [rows] = await pool.execute(
      `SELECT * FROM parenting_confidence_templates
  ...
  if (!template) throw err(404, 'Parenting Confidence template not found');

  const [domains] = await pool.execute(
    `SELECT * FROM parenting_confidence_template_domains
...
async function loadResponses(assessmentId) {
  const [rows] = await pool.execute(
    `SELECT * FROM parenting_confidence_responses WHERE assessment_id = ?`,
    [assessmentId]
  );
  return (rows || []).map((r) => ({
    domainKey: r.domain_key,
    currentCapacityScore:
      r.current_capacity_score == null ? null : Number(r.current_capacity_score),
    ...
    demandLevelScore: r.demand_level_score == null ? null : Number(r.demand_level_score),
...
async function hydrateAssessment(row) {
  ...
  if (Number(row.template_id) !== Number(template.id)) {
    const [trows] = await pool.execute(
      `SELECT * FROM parenting_confidence_templates WHERE id = ? LIMIT 1`,
      [row.template_id]
    );
    if (trows?.[0]) {
      const [domains] = await pool.execute(
  ...
  const participantVersion = row.participant_version || 'general-caregiver';
  ...
  const summary =
    ...
    buildParentingConfidenceSummary(template, responses, {
      mode,
      participantVersion,
      priorityKeys
    });
  ...
  return {
    ...
    parentingConfidenceIndex:
      row.parenting_confidence_index == null
        ? summary.parentingConfidenceIndex
        : Number(row.parenting_confidence_index),
...
export async function getAssessmentById(id) {
  const [rows] = await pool.execute(
    `SELECT * FROM parenting_confidence_assessments WHERE id = ? LIMIT 1`,
    [Number(id)]
  );
...
export async function getAssessmentByToken(token) {
  const [rows] = await pool.execute(
    `SELECT * FROM parenting_confidence_assessments WHERE access_token = ? LIMIT 1`,
    [String(token || '')]
  );
  return hydrateAssessment(rows?.[0] || null);
}
...
export async function createAssessment({
  ...
  mode = 'full',
  participantVersion = 'general-caregiver',
  context = {}
} = {}) {
  ...
  const [result] = await pool.execute(
    `INSERT INTO parenting_confidence_assessments
      (agency_id, template_id, template_version, participant_user_id, client_id,
       coach_user_id, counselor_user_id, mode, participant_version, status,
...
export async function upsertDomainResponse({
  ...
  const nextDemand =
    demandLevelScore === undefined ? existing?.demandLevelScore ?? null : demandLevelScore;
  const nextChips =
    reflectionChips === undefined
  ...
  await pool.execute(
    `INSERT INTO parenting_confidence_responses
      (assessment_id, domain_key, current_capacity_score, personal_importance_score,
       support_need_score, demand_level_score, reflection_chips_json, current_supports_json,
    ...
    [
      Number(assessmentId),
      key,
      nextPrefer || nextSeason === 'not-relevant' ? null : nextCapacity,
      nextImportance,
      nextSupportNeed,
      nextDemand,
  ...
  if (supportTriggers.includes(nextPref)) {
    ...
    await pool.execute(
      `INSERT INTO parenting_confidence_support_requests
...
export async function updatePlans({
  ...
  await pool.execute(
    `UPDATE parenting_confidence_assessments
...
export async function completeAssessment({
  ...
  const summary = buildParentingConfidenceSummary(assessment.template, assessment.responses, {
    mode: assessment.mode,
    participantVersion: assessment.participantVersion,
  ...
  try {
    ...
    scheduleDeliverableGeneration({ family: 'parenting_confidence', assessment: __completed });
  } catch (e) {
    console.warn('[parenting_confidence] deliverable hook failed', e?.message || e);
  }
  return __completed;
}
...
export async function listSubjectAssessments({ participantUserId = null, clientId = null } = {}) {
  ...
  const [rows] = await pool.execute(
    `SELECT id, status, mode, participant_version, parenting_confidence_index, completed_at, created_at
     FROM parenting_confidence_assessments
