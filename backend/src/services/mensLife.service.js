...
export async function getDefaultTemplate({ agencyId = null } = {}) {
  ...
  if (agencyId) {
    [rows] = await pool.execute(
      `SELECT * FROM mens_life_templates
  ...
  if (!template) throw err(404, "Men's Life template not found");

  const [domains] = await pool.execute(
    `SELECT * FROM mens_life_template_domains
...
async function loadResponses(assessmentId) {
  const [rows] = await pool.execute(
    `SELECT * FROM mens_life_responses WHERE assessment_id = ?`,
    [assessmentId]
  );
  return (rows || []).map((r) => ({
    domainKey: r.domain_key,
    currentStrengthScore:
      r.current_strength_score == null ? null : Number(r.current_strength_score),
    personalImportanceScore:
      r.personal_importance_score == null ? null : Number(r.personal_importance_score),
    currentMomentumScore:
      r.current_momentum_score == null ? null : Number(r.current_momentum_score),
    roleDemandScore: r.role_demand_score == null ? null : Number(r.role_demand_score),
...
async function hydrateAssessment(row) {
  ...
  if (Number(row.template_id) !== Number(template.id)) {
    const [trows] = await pool.execute(
      `SELECT * FROM mens_life_templates WHERE id = ? LIMIT 1`,
      [row.template_id]
    );
    if (trows?.[0]) {
      const [domains] = await pool.execute(
  ...
  const participantVersion = row.participant_version || 'married-men';
  ...
  const summary =
    ...
    buildMensLifeSummary(template, responses, {
      mode,
      participantVersion,
      priorityKeys
    });
  ...
  return {
    ...
    summary,
    mensLifeIndex:
      row.mens_life_index == null ? summary.mensLifeIndex : Number(row.mens_life_index),
    importanceWeightedIndex:
      row.importance_weighted_index == null
        ? summary.importanceWeightedLifeIndex
        : Number(row.importance_weighted_index),
    selectedPriorities: priorityKeys,
    developmentPlans: parseJson(row.development_plans_json, []) || [],
    weeklyCommitments: parseJson(row.weekly_commitments_json, []) || [],
    accountabilityPlans: parseJson(row.accountability_plans_json, []) || [],
    template,
...
export async function getAssessmentById(id) {
  const [rows] = await pool.execute(
    `SELECT * FROM mens_life_assessments WHERE id = ? LIMIT 1`,
    [Number(id)]
  );
...
export async function getAssessmentByToken(token) {
  const [rows] = await pool.execute(
    `SELECT * FROM mens_life_assessments WHERE access_token = ? LIMIT 1`,
    [String(token || '')]
  );
  return hydrateAssessment(rows?.[0] || null);
}
...
export async function createAssessment({
  ...
  mentorUserId = null,
  spiritualLeaderUserId = null,
  mode = 'full',
  participantVersion = 'married-men',
  context = {}
} = {}) {
  ...
  const [result] = await pool.execute(
    `INSERT INTO mens_life_assessments
      (agency_id, template_id, template_version, participant_user_id, client_id,
       coach_user_id, counselor_user_id, mentor_user_id, spiritual_leader_user_id,
...
export async function upsertDomainResponse({
  ...
  validateScore(currentStrengthScore, 'Current strength');
  validateScore(personalImportanceScore, 'Personal importance');
  validateScore(currentMomentumScore, 'Current momentum');
  validateScore(roleDemandScore, 'Role demand');

  const existing = assessment.responses.find((r) => r.domainKey === key);
  const nextStrength =
    currentStrengthScore === undefined
      ? existing?.currentStrengthScore ?? null
      : currentStrengthScore;
  ...
  const nextMomentum =
    currentMomentumScore === undefined
      ? existing?.currentMomentumScore ?? null
      : currentMomentumScore;
  const nextDemand =
    roleDemandScore === undefined ? existing?.roleDemandScore ?? null : roleDemandScore;
  const nextChips =
    reflectionChips === undefined
  ...
  await pool.execute(
    `INSERT INTO mens_life_responses
      (assessment_id, domain_key, current_strength_score, personal_importance_score,
       current_momentum_score, role_demand_score, reflection_chips_json, current_supports_json,
     ...
     ON DUPLICATE KEY UPDATE
       current_strength_score = VALUES(current_strength_score),
       personal_importance_score = VALUES(personal_importance_score),
       current_momentum_score = VALUES(current_momentum_score),
       role_demand_score = VALUES(role_demand_score),
    ...
    [
      Number(assessmentId),
      key,
      nextPrefer || nextSeason === 'not-relevant' ? null : nextStrength,
      nextImportance,
      nextMomentum,
      nextDemand,
  ...
  if (supportTriggers.includes(nextPref)) {
    ...
    await pool.execute(
      `INSERT INTO mens_life_support_requests
...
export async function updatePlans({
  ...
  await pool.execute(
    `UPDATE mens_life_assessments
     SET selected_priorities_json = ?,
         development_plans_json = ?,
         weekly_commitments_json = ?,
         accountability_plans_json = ?,
         context_json = ?,
         updated_at = CURRENT_TIMESTAMP
     WHERE id = ?`,
    [
      ...
      JSON.stringify(
        developmentPlans === undefined ? assessment.developmentPlans : developmentPlans || []
      ),
      JSON.stringify(
        weeklyCommitments === undefined
      ...
      JSON.stringify(
        accountabilityPlans === undefined
          ? assessment.accountabilityPlans
          : accountabilityPlans || []
      ),
      JSON.stringify(
...
export async function completeAssessment({
  ...
  const summary = buildMensLifeSummary(assessment.template, assessment.responses, {
    mode: assessment.mode,
    participantVersion: assessment.participantVersion,
  ...
  await pool.execute(
    ...
    [
      JSON.stringify(priorities),
      JSON.stringify(developmentPlans || assessment.developmentPlans || []),
      JSON.stringify(summary),
      summary.mensLifeIndex,
      summary.importanceWeightedLifeIndex,
      Number(assessmentId)
    ]
  );
...
export async function listSubjectAssessments({ participantUserId = null, clientId = null } = {}) {
  ...
  const [rows] = await pool.execute(
    `SELECT id, status, mode, participant_version, mens_life_index, completed_at, created_at
     FROM mens_life_assessments
