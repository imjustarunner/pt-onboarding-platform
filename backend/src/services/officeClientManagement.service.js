...
function deriveBucket(row) {
  ...
  if (!row.providerId) return 'prospective';
  if (
...
/**
 * Agency-scoped office client roster.
 */
export async function listOfficeClients({
  ...
  if (!aid) return { clients: [], aggregates: emptyAggregates() };

  const typePlaceholders = OFFICE_CLIENT_TYPES.map(() => '?').join(',');
  const params = [aid, ...OFFICE_CLIENT_TYPES];
  let sql = `
    SELECT c.id, c.initials, c.full_name, c.identifier_code, c.client_type, c.status,
           ...
           c.guardian_portal_enabled, c.waitlist_started_at, c.client_status_id,
  ...
  try {
    ...
    const [r] = await pool.execute(
      `SELECT c.id, c.initials, c.full_name, c.identifier_code, c.client_type, c.status,
              c.contact_phone, c.submission_date, c.source, c.intake_preferences_json,
              c.created_at, c.provider_id, c.client_status_id,
              cs.status_key AS client_status_key, cs.label AS client_status_label,
              u.first_name AS provider_first, u.last_name AS provider_last
       FROM clients c
       LEFT JOIN client_statuses cs ON cs.id = c.client_status_id
  ...
  const [guardiansMap, unitsMap, nextApptMap, waitlistNotesMap, providerNameById] = await Promise.all([
...
function buildAggregates(source) {
  ...
  for (const c of source) {
    ...
    if (!c.providerId) agg.unassigned += 1;
...
export async function listHubProviders({ agencyId }) {
  const aid = Number(agencyId);
  if (!aid) return [];
  try {
    const [rows] = await pool.execute(
...
export async function getOfficeHubSummary({ agencyId }) {
  const aid = Number(agencyId);
  if (!aid) {
    return {
  ...
  const [{ clients, aggregates }, pending, providers] = await Promise.all([
    listOfficeClients({ agencyId: aid, bucket: 'all', limit: 400 }),
    listPendingOfficeClients({ agencyId: aid }),
    listHubProviders({ agencyId: aid })
  ]);
  ...
  try {
    const appts = await Appointment.listForAgencyInWindow({
      agencyId: aid,
  ...
  try {
    ...
    const referred = await listPendingAcceptanceClients({ agencyId: aid });
    acceptancePending = (referred || []).length;
  } catch {
    acceptancePending = 0;
  }
