...
export async function billingWorkspace(user, query = {}, deps = dependencies) {
  ...
  try {
    [counts] = await deps.clinical.execute(`SELECT agency_id, claim_lifecycle, COUNT(*) AS count FROM clinical_claims
  ...
  if (query.status && query.status !== 'all') {
    ...
    where += ` AND claim_lifecycle IN (${filters[query.status].map(() => '?').join(',')})`; params.push(...filters[query.status]);
  }
  ...
  const [[totalRows], [rows]] = await Promise.all([
    ...
    deps.clinical.execute(`SELECT id, agency_id, clinical_note_id, clinical_session_id, client_id, claim_number, payer_name,
      claim_lifecycle, claimmd_last_status, amount_cents, currency_code, date_of_service, updated_at
      FROM clinical_claims WHERE ${where} ORDER BY CASE claim_lifecycle WHEN 'rejected' THEN 0 WHEN 'denied' THEN 1
