...
import { joinUrlForTeamMeeting, joinUrlForSupervision } from '../utils/joinToken.js';
import { workTitle, calendarWindow } from './calendarPublicationPolicy.js';
...
export async function workCalendarEvents(userId,agencyId,from,to) {
  ...
  const base=await tenantMeetingBase(agencyId);
  const [events]=await pool.execute(`SELECT p.id,p.kind,p.client_id,c.initials AS client_initials,p.is_private,p.start_at,p.end_at,p.start_date,p.end_date,p.all_day,p.updated_at,
  ...
  const output=events.map(e=>({key:`work:${agencyId}:event:${e.id}`,title:workTitle(e),start:e.start_at,end:e.end_at,updatedAt:e.updated_at,
    ...(e.all_day?{startDate:dateOnly(e.start_date),endDate:dateOnly(e.end_date)}:{}),
    url:!e.is_private && ['TEAM_MEETING','HUDDLE'].includes(e.kind)?joinUrlForTeamMeeting(base,e.participant_join_token || e.join_token || e.id):null}));
  const [office]=await pool.execute(`SELECT o.id,o.start_at,o.end_at,o.updated_at,o.client_id,c.initials AS client_initials,l.name,l.street_address,l.timezone,l.events_stored_utc,o.appointment_type_code,o.appointment_subtype_code
    FROM office_events o JOIN office_locations l ON l.id=o.office_location_id LEFT JOIN clients c ON c.id=o.client_id AND c.agency_id=?
  ...
  for(const o of office){
    const utc=v=>{if(o.events_stored_utc)return v;const d=new Date(v);return zonedWallTimeToUtc({year:d.getUTCFullYear(),month:d.getUTCMonth()+1,day:d.getUTCDate(),hour:d.getUTCHours(),minute:d.getUTCMinutes(),timeZone:o.timezone || 'America/Denver'});};
    const s=utc(o.start_at),e=utc(o.end_at);if(e<=start || s>=end)continue;
    output.push({key:`work:${agencyId}:office:${o.id}`,title:workTitle({...o,kind:o.client_id?'SESSION':'OFFICE'}),start:s,end:e,location:[o.name,o.street_address].filter(Boolean).join(' · '),updatedAt:o.updated_at});
  }
  const [supervision]=await pool.execute(`SELECT s.id,s.start_at,s.end_at,s.updated_at,s.modality,s.location_text,s.participant_join_token,s.join_token
    FROM supervision_sessions s WHERE s.agency_id=? AND s.status='SCHEDULED'
  ...
  output.push(...supervision.map(s=>({key:`work:${agencyId}:supervision:${s.id}`,title:'Supervision',start:s.start_at,end:s.end_at,updatedAt:s.updated_at,
    url:['VIRTUAL','VIDEO','TELEHEALTH'].includes(String(s.modality).toUpperCase())?joinUrlForSupervision(base,s.participant_join_token || s.join_token || s.id):null})));
