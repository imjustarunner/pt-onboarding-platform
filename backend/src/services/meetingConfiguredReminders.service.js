  ...
  for(const event of events)for(const reminder of meetingReminderSchedule(event)) {
    ...
    for(const user of users) {
      ...
      try {
        ...
        const result=await sendNotificationEmail({agencyId:event.agency_id,triggerKey:'meeting_join_reminder',to:recipientIdentity.email,replyToOverride:await meetingReplyTo(event),...content,subject:`Meeting reminder: ${event.title}`,userId:user.id,templateType:'meeting_join_reminder',source:'auto'});
