...
export async function getDefaultTemplate({ agencyId = null } = {}) {
  ...
  if (agencyId) {
    [rows] = await pool.execute(
      `SELECT * FROM burden_purpose_templates
  ...
  if (!template) throw err(404, 'Burden & Purpose template not found');

  const [domains] = await pool.execute(
    `SELECT * FROM burden_purpose_template_domains
...
async function loadResponses(assessmentId) {
  const [rows] = await pool.execute(
    `SELECT * FROM burden_purpose_responses WHERE assessment_id = ?`,
    [assessmentId]
  );
  return (rows || []).map((r) => ({
    domainKey: r.domain_key,
    currentPracticeScore:
      r.current_practice_score == null ? null : Number(r.current_practice_score),
    personalImportanceScore:
      r.personal_importance_score == null ? null : Number(r.personal_importance_score),
    sustainableCapacityScore:
      r.sustainable_capacity_score == null ? null : Number(r.sustainable_capacity_score),
    currentLoadScore: r.current_load_score == null ? null : Number(r.current_load_score),
...
async function hydrateAssessment(row) {
  ...
  if (Number(row.template_id) !== Number(template.id)) {
    const [trows] = await pool.execute(
      `SELECT * FROM burden_purpose_templates WHERE id = ? LIMIT 1`,
      [row.template_id]
    );
    if (trows?.[0]) {
      const [domains] = await pool.execute(
  ...
  const summary =
    ...
    buildBurdenPurposeSummary(template, responses, {
      mode,
      participantVersion,
      priorityKeys
    });
  ...
  return {
    ...
    burdenReadinessIndex:
      row.burden_readiness_index == null
        ? summary.burdenReadinessIndex
        : Number(row.burden_readiness_index),
    ...
    burdenEntries: parseJson(row.burden_entries_json, []) || [],
    template,
...
export async function getAssessmentById(id) {
  const [rows] = await pool.execute(
    `SELECT * FROM burden_purpose_assessments WHERE id = ? LIMIT 1`,
    [Number(id)]
  );
...
export async function getAssessmentByToken(token) {
  const [rows] = await pool.execute(
    `SELECT * FROM burden_purpose_assessments WHERE access_token = ? LIMIT 1`,
    [String(token || '')]
  );
  return hydrateAssessment(rows?.[0] || null);
}
...
export async function createAssessment({
  ...
  const [result] = await pool.execute(
    `INSERT INTO burden_purpose_assessments
...
export async function upsertDomainResponse({
  assessmentId,
  domainKey,
  currentPracticeScore = undefined,
  personalImportanceScore = undefined,
  sustainableCapacityScore = undefined,
  currentLoadScore = undefined,
  ...
  validateScore(currentPracticeScore, 'Current practice');
  validateScore(personalImportanceScore, 'Personal importance');
  validateScore(sustainableCapacityScore, 'Sustainable capacity');
  validateScore(currentLoadScore, 'Current load');

  const existing = assessment.responses.find((r) => r.domainKey === key);
  const nextPractice =
    currentPracticeScore === undefined
      ? existing?.currentPracticeScore ?? null
      : currentPracticeScore;
  ...
  const nextCapacity =
    sustainableCapacityScore === undefined
      ? existing?.sustainableCapacityScore ?? null
      : sustainableCapacityScore;
  const nextLoad =
    currentLoadScore === undefined ? existing?.currentLoadScore ?? null : currentLoadScore;
  const nextChips =
    reflectionChips === undefined
  ...
  await pool.execute(
    `INSERT INTO burden_purpose_responses
      (assessment_id, domain_key, current_practice_score, personal_importance_score,
       sustainable_capacity_score, current_load_score, reflection_chips_json, barriers_json,
     ...
     ON DUPLICATE KEY UPDATE
       current_practice_score = VALUES(current_practice_score),
       personal_importance_score = VALUES(personal_importance_score),
       sustainable_capacity_score = VALUES(sustainable_capacity_score),
       current_load_score = VALUES(current_load_score),
    ...
    [
      Number(assessmentId),
      key,
      nextPrefer || nextSeason === 'not-relevant' ? null : nextPractice,
      nextImportance,
      nextCapacity,
      nextLoad,
...
export async function updatePlans({
  ...
  await pool.execute(
    `UPDATE burden_purpose_assessments
    ...
    [
      ...
      JSON.stringify(
        burdenEntries === undefined ? assessment.burdenEntries : burdenEntries || []
      ),
      JSON.stringify(
...
/** Light weekly check-in stub — appends a dated note without full product surface. */
export async function addWeeklyCheckin({ assessmentId, note = '', pillarKeys = [] } = {}) {
  const assessment = await getAssessmentById(assessmentId);
  if (!assessment) throw err(404, 'Assessment not found');
  const entry = {
    at: new Date().toISOString(),
    note: String(note || '').slice(0, 2000),
    pillarKeys: (pillarKeys || []).map(String).slice(0, 4)
  };
...
export async function completeAssessment({
  ...
  if (missing.length) {
    throw err(
      400,
      `All pillars need a score, Prefer not to answer, or Not Relevant. Missing: ${missing
        .map((d) => d.key)
        .join(', ')}`
    );
  }
  ...
  const summary = buildBurdenPurposeSummary(assessment.template, assessment.responses, {
    mode: assessment.mode,
    participantVersion: assessment.participantVersion,
  ...
  try {
    ...
    scheduleDeliverableGeneration({ family: 'burden_purpose', assessment: __completed });
  } catch (e) {
    console.warn('[burden_purpose] deliverable hook failed', e?.message || e);
  }
  return __completed;
}
...
export async function listSubjectAssessments({ participantUserId = null, clientId = null } = {}) {
  ...
  const [rows] = await pool.execute(
    `SELECT id, status, mode, participant_version, burden_readiness_index, completed_at, created_at
     FROM burden_purpose_assessments
