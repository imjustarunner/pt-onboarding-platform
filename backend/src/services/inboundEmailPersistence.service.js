...
export async function persistInboundEmail({ inboxId, agencyId, deliveryId, conversationId = null, threadId = null, ownerUserId = null, fromEmail, replyToEmail = null, subject, bodyText, to = [], cc = [], inReplyTo = null, referencesHeader = null, receivedAt = new Date(), attachments = [], isGroupEmail = false, internalNote = false }) {
  ...
  try {
    ...
    if (!cid) {
      const [created] = await db.execute(`INSERT INTO communication_conversations (agency_id, inbox_id, channel, subject, status, owner_user_id, external_thread_id, is_unknown_sender) VALUES (?, ?, 'email', ?, 'needs_reply', ?, ?, 1)`, [agencyId, inboxId, subject || '(no subject)', ownerUserId, threadId]);
      cid = created.insertId;
    }
    const [msg] = await db.execute(`INSERT INTO communication_messages (conversation_id, channel, direction, from_json, to_json, cc_json, subject, body_text, internet_message_id, in_reply_to, references_header, is_group_email, is_internal_note, send_status, sent_at) VALUES (?, 'email', 'inbound', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'sent', ?)`, [cid, JSON.stringify({ email: fromEmail, ...(replyToEmail ? {replyTo:replyToEmail} : {}) }), JSON.stringify(to), JSON.stringify(cc), subject || null, bodyText || '', deliveryId, inReplyTo, referencesHeader, isGroupEmail ? 1 : 0, internalNote ? 1 : 0, receivedAt]);
