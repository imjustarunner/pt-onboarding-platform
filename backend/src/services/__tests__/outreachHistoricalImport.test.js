...
describe('DPS spreadsheet school matching', () => {
  it('maps unambiguous aliases and skips crossed-out or unknown schools', () => {
    ...
    assert.equal(hit('Park Hill Academy').school?.name, 'Park Hill Elementary School');
    assert.equal(hit('Brown Middle School').school?.name, 'Brown Elementary School');
    assert.equal(hit('Montview Middle School').school?.name, 'DSST: Montview Middle School');
    assert.equal(hit('Swigert International').school?.name, 'Swigert International School');
    assert.equal(hit('Dora Moore ECE-8').school?.name, 'Dora Moore ECE-8 School');
    assert.equal(hit('Valverde Elementary').school?.name, 'Valverde Elementary School');
    assert.equal(hit('Palmer ECE').school?.name, 'Palmer Elementary School');
