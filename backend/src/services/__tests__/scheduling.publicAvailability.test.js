...
describe('published office availability and profile policy',()=>{
 it('keeps office capacity separate while global closed prevents new-client acceptance',async()=>{const result=await compute();expect(result.inPersonSlots).toHaveLength(1);expect(result.inPersonSlots[0].startAt).toBe('2030-01-07T17:00:00.000Z');expect(publicAcceptance({globalAccepting:false,manual:'waitlist',hasOpenings:true}).status).toBe('waitlist');});
