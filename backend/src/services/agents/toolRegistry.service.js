...
async function resolveSchoolPortalPath(agencyIdScope, schoolId) {
  const [rows] = await pool.execute(
    ...
    [agencyIdScope, schoolId, agencyIdScope, schoolId]
  );
  const row = rows?.[0];
...
/** Search affiliated school orgs by name (union of both affiliations and legacy agency_schools). */
async function searchAffiliatedSchoolsByName(agencyId, query, limit = 10) {
  ...
  const runQuery = async (likeArg) => {
    const [rs] = await pool.execute(
      `SELECT DISTINCT id, name, slug, organization_type FROM (
       ...
       ORDER BY name ASC
       LIMIT ${lim}`,
      [agencyId, likeArg, agencyId, likeArg]
    );
    return rs || [];
  };
