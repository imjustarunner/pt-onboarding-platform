...
function buildSystemPrompt({ includeSearch, agentConfig, user }) {
  ...
  return [
    ...
    '- If a search tool returns MORE THAN ONE result, do NOT call openEntity. Set toolCalls to only the search call.',
    '  The system will automatically display clickable cards for each result so the user can choose.',
    '  In assistantText, briefly name the matches. Example: "Found 2 Twain schools — which one did you mean?"',
    '- If a search returns EXACTLY ONE result, you MAY also call openEntity to open it immediately.',
    '- If a search returns zero results, say so clearly and suggest a refinement.',
    '- CRITICAL: If you have shown disambiguation text in a previous turn (e.g. "Found 2 schools"), you MUST call the search tool again in every subsequent turn until the user has selected one. NEVER reply with only assistantText and empty toolCalls when a disambiguation is pending.',
    '',
    ...
    '- If a search returns multiple matches, do NOT add more toolCalls. The server displays clickable cards. Just name the matches briefly in assistantText.',
    '- When a user follows up on a disambiguation (e.g. "twain elementary" after you already found "Twain Elementary" and "Twain Middle"), call searchSchools with ONLY the distinctive part of the name — never include generic words like "school" or "elementary" in the query string.',
