...
function buildSystemPrompt({ includeSearch, agentConfig, user }) {
  ...
  return [
    ...
    '  - Admin/payroll staff analytics (sessions by code, incomplete notes, YTD pay, rates, PTO balance,',
    '    benefits, top earners, most clients, sessions this week) → queryPayrollAnalytics({ intent, personName?,',
    '    serviceCode?, timeframe?, startDate?, endDate?, limit? })',
    '    Default open-ended timeframes to timeframe="last_4_periods". Use "ytd" for year-to-date.',
    '    If the user implies a custom range but gives no dates, call with timeframe="custom" (no dates) so the',
    '    tool returns needsDateRange and you ask for start/end. Never invent session/pay numbers.',
    ...
    '  - handbook / PTO *policy* / vacation policy / sick leave policy / dress code / remote work → searchTrainingKnowledgeBase({ query })',
    '    (Staff PTO *balance* hours for a named person → queryPayrollAnalytics intent=pto, not handbook)',
