...
const KINDS = new Set(['page_view','section_view','click','profile_open','filter_use','search','scroll_depth','document_view','document_download']);
  ...
  async function report(actor,slug,query={}) {
    ...
    const [[totals], [rows], [daily], [pages], [breakdown]] = await Promise.all([
      db.execute(`SELECT COUNT(DISTINCT visitor_hash) visitors, SUM(event_kind='page_view') views,
        SUM(event_kind IN ('click','profile_open','document_view','document_download')) clicks, SUM(event_kind='section_view') impressions,
        SUM(event_kind='profile_open') profileOpens, SUM(event_kind='filter_use') filters, SUM(event_kind='search') searches,
        SUM(event_kind='document_view') documentViews, SUM(event_kind='document_download') documentDownloads,
      ...
      db.execute(`SELECT DATE_FORMAT(occurred_at,'%Y-%m-%d') day, SUM(event_kind='page_view') views,
        COUNT(DISTINCT visitor_hash) visitors,SUM(event_kind IN ('click','profile_open','document_view','document_download')) clicks
    ...
    const total = Object.fromEntries(['visitors','views','clicks','impressions','profileOpens','filters','searches','documentViews','documentDownloads'].map(k=>[k,Number(totals[0]?.[k]||0)]));
