...
/**
 * Keep older school portal clients safe: recovery only emails an optional link.
 * It never installs a temporary password or forces a password change.
 */
export const sendSchoolStaffResetPassword = (req, res, next) => issueSchoolStaffResetLink(req, res, next);

/** Send recovery to the account holder; never expose their reset token to a coworker. */
export const issueSchoolStaffResetLink = createSchoolStaffRecoveryHandler(userHasOrgOrAffiliatedAgencyAccess);
