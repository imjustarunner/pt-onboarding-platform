...
export const updateUser = async (req, res, next) => {
  try {
    ...
    if (hasPlatformGearAccess !== undefined) {
      if (req.user.role !== 'super_admin') {
        return res.status(403).json({ error: { message: 'Only super admins can grant platform Gear access' } });
      }
      updateData.hasPlatformGearAccess = Boolean(hasPlatformGearAccess);
    }
...
export const getAccountInfo = async (req, res, next) => {
  try {
    ...
    const accountInfo = {
      ...
      hasPlatformGearAccess: !!(user.has_platform_gear_access === 1 || user.has_platform_gear_access === true || user.has_platform_gear_access === '1'),
