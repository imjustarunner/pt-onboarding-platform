...
const m=vi.hoisted(()=>({execute:vi.fn(),event:vi.fn(),artifact:vi.fn(),ensure:vi.fn(),append:vi.fn()}));
...
import {postTeamMeetingTranscriptControl,getTeamMeetingAdmissionStatus,saveTeamMeetingClientTranscript} from '../teamMeetings.controller.js';
...
describe('room-wide transcript opt-in',()=>{
  ...
  it('does not allow ordinary invitees to start transcription',async()=>{
    ...
    expect(res.status).toHaveBeenCalledWith(403);expect(m.execute).not.toHaveBeenCalled();
  });
