      ...
      try {
        ...
        const [countRows] = await pool.execute(
          `SELECT company_event_id,
                  SUM(CASE WHEN (COALESCE(intake_outcome, '') <> 'denied')
                    AND (
                      NOT (
                        COALESCE(intake_outcome, '') = 'accepted'
                        OR (COALESCE(intake_complete, 0) = 1 AND COALESCE(intake_outcome, '') = '')
                      )
                      OR (
                        (
                          COALESCE(intake_outcome, '') = 'accepted'
                  ...
                  SUM(CASE WHEN (
                    ...
                    AND (COALESCE(intake_outcome, '') <> 'denied') THEN 1 ELSE 0 END) AS participants_count
