import * as burdenPurposeService from '../services/burdenPurpose.service.js';

function handleError(res, error) {
  const status = error?.status || 500;
  if (status >= 500) {
    console.error('[burden-purpose]', error);
  }
  return res.status(status).json({
    success: false,
    error: error?.message || 'Burden & Purpose assessment error'
  });
}

export async function getGuestTemplate(req, res) {
  try {
    const template = await burdenPurposeService.getDefaultTemplate({});
...
export async function createGuestAssessment(req, res) {
  try {
    const assessment = await burdenPurposeService.createAssessment({
      agencyId: null,
...
export async function getByToken(req, res) {
  try {
    const assessment = await burdenPurposeService.getAssessmentByToken(req.params.token);
    if (!assessment) {
...
export async function upsertResponseByToken(req, res) {
  try {
    ...
    const updated = await burdenPurposeService.upsertDomainResponse({
      assessmentId: assessment.id,
      domainKey: req.body?.domainKey,
      currentPracticeScore: req.body?.currentPracticeScore ?? req.body?.score,
      personalImportanceScore: req.body?.personalImportanceScore ?? req.body?.importanceScore,
      sustainableCapacityScore:
        req.body?.sustainableCapacityScore ?? req.body?.capacityScore,
      currentLoadScore: req.body?.currentLoadScore ?? req.body?.loadScore,
...
export async function updatePlansByToken(req, res) {
  try {
    ...
    const updated = await burdenPurposeService.updatePlans({
      assessmentId: assessment.id,
      ...
      burdenEntries: req.body?.burdenEntries,
...
export async function completeByToken(req, res) {
  try {
    ...
    const updated = await burdenPurposeService.completeAssessment({
      assessmentId: assessment.id,
      selectedPriorities: req.body?.selectedPriorities || [],
      commitmentPlans: req.body?.commitmentPlans || req.body?.developmentPlans || []
    });
...
export async function getTemplate(req, res) {
  try {
    ...
    const template = await burdenPurposeService.getDefaultTemplate({ agencyId });
    return res.json({ success: true, template });
  } catch (error) {
    return handleError(res, error);
  }
...
export async function getAssessment(req, res) {
  try {
    const assessment = await burdenPurposeService.getAssessmentById(req.params.id);
    if (!assessment) {
      return res.status(404).json({ success: false, error: 'Assessment not found' });
    }
    return res.json({ success: true, assessment });
...
export async function listMine(req, res) {
  try {
    const rows = await burdenPurposeService.listSubjectAssessments({
