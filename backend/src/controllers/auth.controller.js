  ...
  try {
    ...
    const responseData = {
      ...
      user: {
        ...
        group_supervision_eligible: !!(freshUser?.group_supervision_eligible === true || freshUser?.group_supervision_eligible === 1 || freshUser?.group_supervision_eligible === '1'),
...
export const identifyLogin = async (req, res, next) => {
  try {
    ...
    if (!hasSsoPasswordOverride && !googleStartUrl && rescueMode) {
    ...
    if (!hasSsoPasswordOverride && !googleStartUrl && resolvedSlug) {
      try {
    ...
    try {
      ...
      res.json({
        ...
        user: {
          ...
          group_supervision_eligible: !!(fullUser?.group_supervision_eligible === true || fullUser?.group_supervision_eligible === 1 || fullUser?.group_supervision_eligible === '1'),
