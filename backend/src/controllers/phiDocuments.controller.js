...
/**
 * Best-effort backfill of `client_phi_documents` from signed intake submission
 * documents that are already persisted in `intake_submission_documents` but
 * never produced a PHI row (usually because the finalize flow's previous
 * empty-catch swallowed an insert error). Without this the client's
 * Documentation tab shows "No packets found" even though the client signed a
 * full packet — a confusing, confidence-destroying UX.
 *
 * Returns an array of newly-created PHI doc rows (possibly empty).
 */
async function backfillMissingPhiDocsFromIntakeSubmissions({ client, existingDocs = [] } = {}) {
  ...
  // Signed docs that should map into client_phi_documents. We accept either
  // `intake_submissions.client_id = :cid` OR a multi-child link through
  // `intake_submission_clients` so sibling packets backfill too.
  let rows = [];
  try {
    const [result] = await pool.execute(
      `SELECT isd.id AS submission_doc_id,
              isd.intake_submission_id,
              isd.document_template_id,
              isd.signed_pdf_path,
              isd.signed_at,
              s.id AS submission_id,
              s.client_id AS submission_client_id,
              s.signer_email,
              s.ip_address,
              s.intake_link_id,
              dt.name AS template_name,
              il.organization_id AS link_org_id
         FROM intake_submission_documents isd
         JOIN intake_submissions s ON s.id = isd.intake_submission_id
         LEFT JOIN intake_submission_clients isc
                ON isc.intake_submission_id = s.id AND isc.client_id = ?
        ...
        WHERE isd.signed_pdf_path IS NOT NULL
          AND isd.signed_pdf_path <> ''
          AND (s.client_id = ? OR isc.client_id = ?)`,
      [clientId, clientId, clientId]
    );
    rows = result || [];
  } catch (qErr) {
    // Missing tables (fresh schema) — no backfill possible. Swallow silently.
    const code = qErr?.code || '';
    if (code !== 'ER_NO_SUCH_TABLE') {
      console.warn('[phiDocuments] backfill query failed', { clientId, message: qErr?.message });
    }
    return [];
  }
  ...
  const created = [];
  for (const r of rows) {
    const storagePath = String(r.signed_pdf_path || '').trim();
    if (!storagePath) continue;
    if (existingPaths.has(storagePath)) continue;

    const agencyId = Number(
      client?.agency_id || r.link_org_id || 0
    ) || null;
    const schoolOrgId = Number(
      client?.organization_id || client?.school_organization_id || r.link_org_id || agencyId || 0
    ) || null;
    if (!agencyId || !schoolOrgId) {
      // Table requires both NOT NULL; skip rather than blow up.
      continue;
    }
    ...
    try {
      const phiDoc = await ClientPhiDocument.create({
        ...
        agencyId,
        schoolOrganizationId: schoolOrgId,
        intakeSubmissionId: r.intake_submission_id || null,
        storagePath,
        originalName: `${r.template_name || 'Document'} (Signed)`,
      ...
      await PhiDocumentAuditLog.create({
        ...
        action: 'uploaded',
        actorUserId: null,
        actorLabel: 'public_intake_backfill',
        ipAddress: r.ip_address || null,
        metadata: {
          submissionId: r.intake_submission_id || null,
          templateId: r.document_template_id || null,
          backfill: true
        }
      }).catch(() => {});
    } catch (createErr) {
      const msg = String(createErr?.message || '');
      // Unique index on storage_path can legitimately collide if a concurrent
      // request also backfilled — swallow duplicate-key errors quietly.
      if (
        createErr?.code === 'ER_DUP_ENTRY' ||
        /Duplicate entry/i.test(msg) ||
        /uniq_phi_storage_path/i.test(msg)
      ) {
        continue;
      }
      console.warn('[phiDocuments] backfill insert failed', {
        clientId,
        storagePath,
        message: msg
      });
...
export const listClientPhiDocuments = async (req, res, next) => {
  try {
    ...
    // Self-heal: if any signed intake documents exist for this client but did
    // not make it into `client_phi_documents` (historically, earlier versions
    // swallowed the insert error so a failed row never surfaced), create them
    // now so the Documentation tab shows the packet PDFs the client actually
    // signed. This keeps the admin view truthful without requiring a re-run
    // of the intake submission.
    try {
      const healed = await backfillMissingPhiDocsFromIntakeSubmissions({
        client,
        existingDocs: docs
      });
      if (Array.isArray(healed) && healed.length) {
        docs = docs.concat(healed);
        docs.sort((a, b) => {
      ...
      // Self-heal must NEVER block the list endpoint — just log and continue.
      console.warn('[phiDocuments] backfill from intake_submissions failed', {
        clientId,
