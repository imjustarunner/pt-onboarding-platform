...
describe('counseling dependent EN intake', () => {
  it('builds family pages, 18 child pages, review, and unique keys', () => {
    ...
    const about = steps.find((s) => s.id === `${COUNSELING_DEP_STEP_PREFIX}what_brings`);
    const biggest = about.fields.find((f) => f.key === 'biggest_concern_now');
    assert.deepEqual(biggest.showIf, { fieldKey: 'presenting_concerns', equals: [] });
    const psc = steps
  ...
  it('merges with self pages and keeps packet steps', () => {
    ...
    assert.equal(selfCount, 14);
    assert.equal(depCount, 22);
