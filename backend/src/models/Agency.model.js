...
class Agency {
  ...
  static async create(agencyData) {
    ...
    if (hasPortalConfig) {
      insertFields.push('onboarding_team_email', 'support_team_email', 'phone_number', 'phone_extension', 'portal_url', 'theme_settings');
      insertValues.push(
        onboardingTeamEmail || null,
        supportTeamEmail || null,
        phoneNumber || null,
        phoneExtension || null,
    ...
    try {
      ...
      hasFaq = names.has('school_portal_faq_icon_id');
  ...
  static async update(id, agencyData) {
    ...
    if (hasPortalConfig) {
      ...
      if (supportTeamEmail !== undefined) {
        updates.push('support_team_email = ?');
        values.push(supportTeamEmail || null);
      }
    ...
    if (tenantAvailableAgencyFeaturesJson !== undefined) {
      let hasTenantAvail = false;
      try {
        const [columns] = await pool.execute(
          "SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'agencies' AND COLUMN_NAME = 'tenant_available_agency_features_json'"
        );
        hasTenantAvail = columns.length > 0;
      } catch (e) {
        hasTenantAvail = false;
      }
      if (hasTenantAvail) {
        updates.push('tenant_available_agency_features_json = ?');
        values.push(
          tenantAvailableAgencyFeaturesJson ? JSON.stringify(tenantAvailableAgencyFeaturesJson) : null
        );
      }
    }
