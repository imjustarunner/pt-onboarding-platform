...
class AgencyNotificationTriggerSetting {
  static async listForAgency(agencyId) {
    const [rows] = await pool.execute(
      `SELECT id, agency_id, trigger_key, enabled, channels_json, recipients_json, created_at, updated_at
  ...
  static async upsert({ agencyId, triggerKey, enabled = null, channels = null, recipients = null }) {
    ...
    await pool.execute(
      `INSERT INTO agency_notification_trigger_settings
        (agency_id, trigger_key, enabled, channels_json, recipients_json)
       VALUES (?, ?, ?, ?, ?)
      ...
      [agencyId, triggerKey, enabledValue, channelsJson, recipientsJson]
    );
