...
class PlatformBranding {
  static async get() {
    try {
      ...
      // Check if additional dashboard action icon overrides exist (optional)
      let hasProgramOverviewIcon = false;
      let hasProviderAvailabilityDashboardIcon = false;
      let hasExecutiveReportIcon = false;
      try {
        const [cols] = await pool.execute(
          "SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'platform_branding' AND COLUMN_NAME IN ('program_overview_icon_id','provider_availability_dashboard_icon_id','executive_report_icon_id')"
        );
        const names = new Set((cols || []).map((c) => c.COLUMN_NAME));
      ...
      try {
        ...
        const programOverviewJoins = hasProgramOverviewIcon
          ? `
          LEFT JOIN icons pov_i ON pb.program_overview_icon_id = pov_i.id`
          : '';
        ...
        const providerAvailabilityJoins = hasProviderAvailabilityDashboardIcon
          ? `
          LEFT JOIN icons pad_i ON pb.provider_availability_dashboard_icon_id = pad_i.id`
          : '';
  ...
  static async update(brandingData, userId) {
    ...
    if (existing.id) {
      ...
      if (hasDashboardIcons) {
        ...
        if (programOverviewIconId !== undefined) {
          try {
            const [cols] = await pool.execute(
              "SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'platform_branding' AND COLUMN_NAME = 'program_overview_icon_id'"
            );
            if ((cols || []).length > 0) {
              updates.push('program_overview_icon_id = ?');
              values.push(programOverviewIconId ?? null);
            } else {
              const err = new Error(
                'Cannot save Program Overview icon: database missing platform_branding.program_overview_icon_id. Run database/migrations/313_dashboard_action_icons_program_overview_provider_availability_executive_report.sql.'
              );
              err.status = 409;
              throw err;
            }
            ...
            if (e?.status) throw e;
            console.warn('PlatformBranding.update: Error checking for program_overview_icon_id column:', e.message);
          }
        }

        if (providerAvailabilityDashboardIconId !== undefined) {
          try {
            const [cols] = await pool.execute(
              "SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'platform_branding' AND COLUMN_NAME = 'provider_availability_dashboard_icon_id'"
            );
            if ((cols || []).length > 0) {
              updates.push('provider_availability_dashboard_icon_id = ?');
              values.push(providerAvailabilityDashboardIconId ?? null);
            } else {
              const err = new Error(
                'Cannot save Provider Availability icon: database missing platform_branding.provider_availability_dashboard_icon_id. Run database/migrations/313_dashboard_action_icons_program_overview_provider_availability_executive_report.sql.'
              );
              err.status = 409;
              throw err;
            }
            ...
            if (e?.status) throw e;
            console.warn('PlatformBranding.update: Error checking for provider_availability_dashboard_icon_id column:', e.message);
          }
        }

        if (executiveReportIconId !== undefined) {
          try {
            const [cols] = await pool.execute(
              "SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'platform_branding' AND COLUMN_NAME = 'executive_report_icon_id'"
            );
            if ((cols || []).length > 0) {
              updates.push('executive_report_icon_id = ?');
              values.push(executiveReportIconId ?? null);
            } else {
              const err = new Error(
                'Cannot save Executive Report icon: database missing platform_branding.executive_report_icon_id. Run database/migrations/313_dashboard_action_icons_program_overview_provider_availability_executive_report.sql.'
              );
              err.status = 409;
              throw err;
            }
            ...
            if (e?.status) throw e;
            console.warn('PlatformBranding.update: Error checking for executive_report_icon_id column:', e.message);
          }
        }
      ...
      if (
        ...
        schoolPortalUploadPacketIconId !== undefined ||
        schoolPortalPublicDocumentsIconId !== undefined ||
        schoolPortalAnnouncementsIconId !== undefined ||
        schoolPortalEventsIconId !== undefined ||
        schoolPortalDigitalFormsIconId !== undefined ||
        schoolPortalCalendarIconId !== undefined
      ) {
        try {
          const [cols] = await pool.execute(
          ...
          const hasPublicDocs = names.has('school_portal_public_documents_icon_id');
          const hasAnnouncements = names.has('school_portal_announcements_icon_id');
          const hasEvents = names.has('school_portal_events_icon_id');
          const hasDigitalForms = names.has('school_portal_digital_forms_icon_id');
          const hasCalendar = names.has('school_portal_calendar_icon_id');
          if (schoolPortalSchoolStaffIconId !== undefined && !hasSchoolPortalStaffIcon) {
            const err = new Error(
              'Cannot save School staff icon: database missing platform_branding.school_portal_school_staff_icon_id. Run database/migrations/291_school_portal_school_staff_card_icon.sql.'
            );
            err.status = 409;
            throw err;
          }
          if (schoolPortalParentQrIconId !== undefined && !hasParentQr) {
            const err = new Error(
              'Cannot save Parent QR code icon: database missing platform_branding.school_portal_parent_qr_icon_id. Run database/migrations/293_school_portal_parent_packet_cards_icons.sql.'
            );
            err.status = 409;
            throw err;
          }
          if (schoolPortalParentSignIconId !== undefined && !hasParentSign) {
            const err = new Error(
              'Cannot save Parent fill + sign icon: database missing platform_branding.school_portal_parent_sign_icon_id. Run database/migrations/293_school_portal_parent_packet_cards_icons.sql.'
            );
            err.status = 409;
            throw err;
          }
          if (schoolPortalUploadPacketIconId !== undefined && !hasUploadPacket) {
            const err = new Error(
              'Cannot save Upload packet icon: database missing platform_branding.school_portal_upload_packet_icon_id. Run database/migrations/293_school_portal_parent_packet_cards_icons.sql.'
            );
            err.status = 409;
            throw err;
          }
          if (schoolPortalPublicDocumentsIconId !== undefined && !hasPublicDocs) {
            const err = new Error(
              'Cannot save Documents icon: database missing platform_branding.school_portal_public_documents_icon_id. Run database/migrations/303_school_portal_public_documents_card_icon.sql.'
            );
            err.status = 409;
            throw err;
          }
          if (schoolPortalAnnouncementsIconId !== undefined && !hasAnnouncements) {
            const err = new Error(
              'Cannot save Announcements icon: database missing platform_branding.school_portal_announcements_icon_id. Run database/migrations/336_school_portal_announcements_and_notifications.sql.'
            );
            err.status = 409;
            throw err;
          }
          if (schoolPortalEventsIconId !== undefined && !hasEvents) {
            const err = new Error(
              'Cannot save Events icon: database missing platform_branding.school_portal_events_icon_id. Run database/migrations/959_school_portal_events_and_digital_forms_icons.sql.'
            );
            err.status = 409;
            throw err;
          }
          if (schoolPortalDigitalFormsIconId !== undefined && !hasDigitalForms) {
            const err = new Error(
              'Cannot save Manage digital forms icon: database missing platform_branding.school_portal_digital_forms_icon_id. Run database/migrations/959_school_portal_events_and_digital_forms_icons.sql.'
            );
            err.status = 409;
            throw err;
          }
          if (schoolPortalCalendarIconId !== undefined && !hasCalendar) {
            const err = new Error(
              'Cannot save School calendar icon: database missing platform_branding.school_portal_calendar_icon_id. Run database/migrations/960_school_event_district_broadcast_and_calendar_icon.sql.'
            );
            err.status = 409;
            throw err;
          }
          ...
          // If a specific migration is required, surface it to the caller.
          if (e?.status) throw e;
      ...
      // Program Overview quick-action icon (optional) - for INSERT
      if (programOverviewIconId !== undefined) {
        try {
          const [cols] = await pool.execute(
            "SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'platform_branding' AND COLUMN_NAME = 'program_overview_icon_id'"
          ...
          if ((cols || []).length > 0) {
            insertFields.push('program_overview_icon_id');
            insertValues.push(programOverviewIconId || null);
          }
        } catch {
          // ignore
        }
      ...
      // Provider Availability quick-action icon (optional) - for INSERT
      if (providerAvailabilityDashboardIconId !== undefined) {
        try {
          const [cols] = await pool.execute(
            "SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'platform_branding' AND COLUMN_NAME = 'provider_availability_dashboard_icon_id'"
          ...
          if ((cols || []).length > 0) {
            insertFields.push('provider_availability_dashboard_icon_id');
            insertValues.push(providerAvailabilityDashboardIconId || null);
          }
        } catch {
          // ignore
        }
      ...
      // Executive Report quick-action icon (optional) - for INSERT
      if (executiveReportIconId !== undefined) {
        try {
          const [cols] = await pool.execute(
            "SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'platform_branding' AND COLUMN_NAME = 'executive_report_icon_id'"
          ...
          if ((cols || []).length > 0) {
            insertFields.push('executive_report_icon_id');
            insertValues.push(executiveReportIconId || null);
          }
        } catch {
          // ignore
        }
