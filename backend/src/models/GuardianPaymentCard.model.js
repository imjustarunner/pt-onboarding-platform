...
export default class GuardianPaymentCard {
  static async create({
    ...
    // Clear existing default for this guardian+agency before setting a new one.
    if (isDefault) {
      await pool.query(
    ...
    const [result] = await pool.query(
      `INSERT INTO guardian_payment_cards
        (guardian_user_id, agency_id, payment_provider,
         qb_payment_customer_id, qb_card_id,
      ...
      [
        guardianUserId, agencyId, paymentProvider,
        qbPaymentCustomerId || null, qbCardId || null,
