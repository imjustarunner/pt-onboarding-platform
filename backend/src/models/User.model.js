...
class User {
  ...
  static async findByWorkEmail(workEmail) {
    ...
    try {
      ...
      const dbName = process.env.DB_NAME || 'onboarding_stage';
      const [columns] = await pool.execute(
        "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'users' AND COLUMN_NAME IN ('work_email', 'personal_email', 'has_supervisor_privileges', 'personal_phone', 'work_phone', 'work_phone_extension', 'credential')",
        [dbName]
      );
      const existingColumns = columns.map(c => c.COLUMN_NAME);
      if (existingColumns.includes('work_email')) query += ', work_email';
  ...
  static async findById(id) {
    ...
    try {
      ...
      if (existingColumns.includes('has_outreach_access')) query += ', has_outreach_access';
      if (existingColumns.includes('has_platform_gear_access')) query += ', has_platform_gear_access';
      if (existingColumns.includes('has_medical_records_release_access')) query += ', has_medical_records_release_access';
      if (existingColumns.includes('has_games_access')) query += ', has_games_access';
      if (existingColumns.includes('is_hourly_worker')) query += ', is_hourly_worker';
      if (existingColumns.includes('hourly_dual_rate_enabled')) query += ', hourly_dual_rate_enabled';
      if (existingColumns.includes('employee_id')) query += ', employee_id';
      if (existingColumns.includes('sso_password_override')) query += ', sso_password_override';
      if (existingColumns.includes('login_is_group_email')) query += ', login_is_group_email';
  ...
  static async update(id, userData) {
    ...
    if (credential !== undefined) {
      try {
        const [columns] = await pool.execute(
          "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'credential'"
        );
        if (columns.length > 0) {
          updates.push('credential = ?');
          values.push(credential ? String(credential).trim() : null);
        }
      } catch {
        // ignore (older DBs)
      }
    ...
    if (hasOutreachAccess !== undefined) {
      try {
        const dbName = process.env.DB_NAME || 'onboarding_stage';
        const [columns] = await pool.execute(
          "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'users' AND COLUMN_NAME = 'has_outreach_access'",
          [dbName]
        );
        if (columns.length > 0) {
          updates.push('has_outreach_access = ?');
          values.push(hasOutreachAccess ? 1 : 0);
        }
      } catch (err) {
        console.warn('has_outreach_access column check failed:', err.message);
      }
    }

    if (hasPlatformGearAccess !== undefined) {
      try {
        const dbName = process.env.DB_NAME || 'onboarding_stage';
        const [columns] = await pool.execute(
          "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'users' AND COLUMN_NAME = 'has_platform_gear_access'",
          [dbName]
        );
        if (columns.length > 0) {
          updates.push('has_platform_gear_access = ?');
          values.push(hasPlatformGearAccess ? 1 : 0);
        }
      } catch (err) {
        console.warn('has_platform_gear_access column check failed:', err.message);
      }
    }
    ...
    // Games access entitlement (per-user billing feature)
    if (hasGamesAccess !== undefined) {
      try {
        const dbName = process.env.DB_NAME || 'onboarding_stage';
        const [columns] = await pool.execute(
          "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'users' AND COLUMN_NAME = 'has_games_access'",
          [dbName]
        );
        if (columns.length > 0) {
          updates.push('has_games_access = ?');
          values.push(hasGamesAccess ? 1 : 0);
        }
      } catch (err) {
        console.warn('has_games_access column check failed:', err.message);
      }
    }
    ...
    if (providerAcceptingNewClients !== undefined) {
      try {
        ...
        if (String(err?.message || '').includes('provider_accepting_new_clients')) throw err;
        console.warn('provider_accepting_new_clients update skipped:', err.message);
      }
    }

    // Office Availability (Client Exchange / office routing) — independent of schedule slots.
    if (inOfficeAvailable !== undefined) {
      try {
        const dbName = process.env.DB_NAME || 'onboarding_stage';
        const [columns] = await pool.execute(
          "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'users' AND COLUMN_NAME = 'in_office_available'",
          [dbName]
        );
        if (columns.length > 0) {
          updates.push('in_office_available = ?');
          values.push(inOfficeAvailable ? 1 : 0);
        }
      } catch (err) {
        console.warn('in_office_available update skipped:', err.message);
      }
    }
