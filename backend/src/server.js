...
if (!isBootstrap) {
  ...
  (async () => {
    try {
      ...
      const [[deliveryContextCol]] = await pool.execute(
        `SELECT 1
           FROM information_schema.COLUMNS
          WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME = 'learning_class_sessions'
            AND COLUMN_NAME = 'delivery_context'`
      );
      if (!deliveryContextCol) {
        await pool.execute(
          `ALTER TABLE learning_class_sessions
             ADD COLUMN delivery_context VARCHAR(32) NULL DEFAULT NULL AFTER session_subtype`
        );
        console.log('[startup] Migration 743 applied: delivery_context added to learning_class_sessions');
      }
      ...
      console.warn('[startup] Migration 743 check skipped:', err.message);
    }
  })();
  ...
  (async () => {
    try {
      ...
      const [[whiteboardCol]] = await pool.execute(
        `SELECT 1
           FROM information_schema.COLUMNS
          WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME = 'learning_class_session_in_person_plans'
            AND COLUMN_NAME = 'whiteboard_json'`
      );
      if (!whiteboardCol) {
        await pool.execute(
          `ALTER TABLE learning_class_session_in_person_plans
             ADD COLUMN whiteboard_json LONGTEXT NULL AFTER layout_prefs_json`
        );
        console.log('[startup] Migration 744 applied: whiteboard_json added to learning_class_session_in_person_plans');
      }

      const [[shareWhiteboardCol]] = await pool.execute(
        `SELECT 1
           FROM information_schema.COLUMNS
          WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME = 'learning_class_session_in_person_plans'
            AND COLUMN_NAME = 'share_whiteboard_with_guardian'`
      );
      if (!shareWhiteboardCol) {
        await pool.execute(
          `ALTER TABLE learning_class_session_in_person_plans
             ADD COLUMN share_whiteboard_with_guardian TINYINT(1) NOT NULL DEFAULT 0 AFTER whiteboard_json`
        );
        console.log('[startup] Migration 744 applied: share_whiteboard_with_guardian added to learning_class_session_in_person_plans');
      }
    } catch (err) {
