...
## Billing (Agency Add-On)

- **Price:** $5 per person per month (or per billing period)
- **Scope:** Momentum List + Momentum Stickies (the full feature set)
- **Gating:** Feature is enabled per agency via feature flag / add-on subscription
- **Implementation notes:**
  - Add `momentumListEnabled` (or similar) to agency `feature_flags` or a dedicated add-ons table
  - Gate visibility: Momentum Stickies overlay + Momentum List rail card only when agency has the add-on
  - Track licensed users for billing (e.g., active employees in agencies with the add-on)
...
1. Gemini CRUD + digest generation
2. Audit logging
3. Sticky → Task promotion
