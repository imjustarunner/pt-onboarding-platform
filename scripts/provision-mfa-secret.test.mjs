...
function fixture({ entry, exists = true, value = key, state = 'ENABLED' } = {}) {
  ...
  const call = (args, options = {}) => {
    ...
    if (args.join(' ').startsWith('secrets versions describe')) return { name: `projects/test/secrets/${MFA_ENV}/versions/3`, state };
...
test('leaves an existing pinned key untouched', () => {
  ...
  assert.equal(f.run().changed, false); assert.equal(f.calls.length, 1);
});
...
test('refuses mismatched existing keys, disabled versions, and malformed keys', () => {
  assert.throws(() => fixture({ entry: { name: MFA_ENV, value: key }, value: Buffer.alloc(32, 8).toString('base64') }).run(), /differ/);
