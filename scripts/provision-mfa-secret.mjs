...
export function provisionMfaSecret({ call, service, region, makeKey = () => randomBytes(32).toString('base64') }) {
  ...
  if (ref?.name && /^[1-9][0-9]*$/.test(ref.key)) return { changed: false, secret: ref.name, version: ref.key };
  ...
  return { changed: true, secret, version };
}
...
function cloudCall(args, { input, raw = false } = {}) {
  ...
  if (result.status !== 0) throw new Error(`Cloud operation failed: ${args.slice(0, 3).join(' ')}. Check the deployment account's permissions. Secret values have not been logged.`);
