...
Use **Voice event** in the FCC header. On an iPad or iPhone, choose **Use keyboard dictation**, tap the microphone on the keyboard, speak, and choose **Fill event form**. Other supported browsers offer **Start speaking**, **Stop listening**, and **Fill event form**. The helper is also available inside a new personal event form. Review the transcribed words before asking it to fill the form. The ordinary **Save to our family** action is the only action that creates an event and its usual calendar projections.

The authenticated `POST /api/family/households/:id/voice/event-draft` endpoint checks the FCC benefit and household membership, reads only that household’s member names, and returns an allowlisted event draft. It receives no workplace calendars, client records, or event history and has no tool execution or write path. Relative dates use the household’s current date and time zone. Missing times remain blank; uncertain member matches, partial dates, unsupported recurrence, and other questions appear for review. A shared device does not identify a speaker from “me.” Only one occurrence is drafted at a time.

iPads/iPhones use OS keyboard dictation rather than WebKit page-level speech capture. Other browsers use opt-in single-utterance speech capture, with an 8-second startup watchdog, a 15-second no-result watchdog, a one-minute maximum, and an explicit manual-entry escape. Focus music pauses before either dictation flow. Capture and pending AI requests are canceled before saving manually or closing the helper. Browsers without recognition can use keyboard dictation or typed text in the same input. The application does not store an audio recording or persist the transcript. Draft interpretation uses the existing private Vertex Gemini configuration (`vertexOnly`, `sensitive`) and is limited to 30 requests per signed-in family user per hour. Model errors retain the words in the open form for retry/manual entry; they never save a partial event. No new migration or tenant feature flag is required.

### Event editor responsiveness

The editor mounts no picture-library images until the gallery is opened, and reveals at most 24 more at a time. Its backdrop avoids live calendar blur. Calendar day layouts and time formatters are cached across clock/gesture updates; background household refresh is deferred while a dialog is open. Once an event save is acknowledged, the normal calendar becomes interactive immediately, independently of the follow-up refresh. Calendar reads time out after 20 seconds and do not disable touch entry. A lost touch pointer capture is released safely. Saved-status notices do not intercept taps on the calendar controls.
