...
ITSCO’s public clinical-supervisor cards exclude Destiny Roberts (477) and Melissa Mendez (538) by the organization’s publication decision. This leaves their app privileges untouched: Destiny remains in training mentors, and Melissa remains in the support team. Founder introductions include expanded editorial biographies drawn from the supplied ITSCO history and published roles; a saved biography overrides that default.

### Paid practicum/internship placements and Rachel inquiries

The program and fair display advertise rolling Fall, Spring, and Summer admission: 3–5 Colorado Springs placements and 2–3 Denver placements, with $24–$30 per hour of direct service for both training phases. `backend/src/scripts/updateItscoPracticumPosting.js` previews the targeted published posting update; `--apply` updates the single active ITSCO placement posting while retaining its existing application key and page design. Careers filters include the combined posting in both cities.

Only the internship-area pages render Rachel's contact form. `POST /api/public/agency-support/itsco/internship-inquiries` requires name, valid email, message, privacy acknowledgment, and the existing website CAPTCHA/rate limits. The server fixes the subject to `Practicum/Internship inquiry`, resolves Rachel's active ITSCO membership and configured work mailbox, saves the encrypted website ticket, assigns it to Rachel, and emails her through the unified ITSCO support sender. No recipient address or sender can be supplied by the visitor. Rachel opens the linked ticket and uses **Reply** to send branded email back to the visitor. Delivery failures retain the assigned ticket and are reported truthfully in the form confirmation and communication delivery logs.
