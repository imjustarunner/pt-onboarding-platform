...
Run `node frontend/scripts/verify-login-branding.cjs` with the same local Vite and Playwright setup. This serves virtual tenant hosts from local Vite and intercepts every API request with fictional data. It verifies the custom-domain timeout login, username identification on a flat custom-domain login, a simulated Google callback through the actual login view, actual logout, signed company-domain handoff memory, explicit opt-out after session refresh, a remembered account's direct Google start link, username-only password memory, scoped account forgetting, platform/tenant logo separation, and saved light/dark preferences after reload for both the logged-out page and welcome briefing. It does not authenticate with Google or access a real user's session.
...
Remembered usernames and Google account display details are stored per portal in the browser, with migration from the previous single-account keys. Passwords and Google credentials are not stored. Remembered accounts remain specific to the browser origin; a different browser needs its own successful Google sign-in. A verified Google session transferred through the signed company-domain handoff can establish the destination portal’s account card when remembering is enabled. The handoff retains sign-in provenance for display without treating it as a fresh Google reauthentication. A saved app username or email alias must still resolve to the same remembered Google account. Forgetting one portal's account must not remove another portal's shortcut.

Flat custom-domain username identification must send the effective portal slug, including for platform administrators whose identify response has no resolved organization. Only server-verified Google sessions may create Google shortcuts. Explicit Forget/uncheck choices persist across session refresh and handoffs. ITSCO’s known legacy navy/blue/orange default is replaced with green/teal for login controls; customized palettes and other tenants are preserved.
