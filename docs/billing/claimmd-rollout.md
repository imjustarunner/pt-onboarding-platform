...
The client-scoped eligibility, other-coverage review, and secondary-claim workflow now replace the unrestricted eligibility payload endpoint. See [eligibility and secondary insurance setup](eligibility-and-secondary-insurance.md) for activation steps, database migrations, validation and remaining payer checks. Account approval settings, payer credentialing, provider phone verification and enrollment paperwork remain Claim.MD/payer actions. The PDF about disabling approval explains an option; it was not treated as authorization to turn off approval.
