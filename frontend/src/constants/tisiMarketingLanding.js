...
const ICON_SVG = {
  ...
  grad: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"><path d="M4 19h16M6 19V10l6-4 6 4v9"/><path d="M10 19v-4h4v4"/></svg>`,
...
export function defaultTisiLandingConfig() {
  return {
    ...
    processSteps: [
      {
        title: 'Connect',
        body: 'Reach out through our secure online form or give us a call.',
        href: '/p/tisi/contact'
      },
      {
        title: 'Get Matched',
        body: "We'll match you with the right therapist for your goals and needs.",
        href: '/p/tisi/get-started'
      },
      {
        title: 'Begin Care',
    ...
    testimonials: [
      {
        text: 'I finally felt understood. The tools I learned here changed how I show up for my family and myself.',
        attribution: 'Client, 34'
      },
      {
        text: 'Our son has more confidence at school and at home. We are grateful for the patience and skill of his counselor.',
        attribution: 'Parent, 42'
      },
      {
        text: 'Performance work here helped me compete with a clearer head — and recover when things got hard.',
        attribution: 'College Athlete, 20'
      }
    ],
    ...
    socialLinks: [
      { label: 'Instagram', short: 'IG', href: '/p/tisi/resources' },
      { label: 'LinkedIn', short: 'in', href: '/p/tisi/resources' },
      { label: 'YouTube', short: 'YT', href: '/p/tisi/resources' },
      { label: 'Spotify', short: '♪', href: '/p/tisi/resources' }
    ],
    primaryNav: [
...
function mergeList(defaults, overrides, mapRow) {
  if (!Array.isArray(overrides) || !overrides.length) return defaults.map((d) => ({ ...d }));
...
export function resolveTisiLandingConfig({ pageMeta, branding } = {}) {
  ...
  const pick = (...vals) => {
    for (const v of vals) {
      ...
      if (s !== '' && s !== undefined) return typeof v === 'string' ? String(v) : v;
    }
    return '';
  };
  ...
  const primaryNavRaw = Array.isArray(b.primaryNav) && b.primaryNav.length ? b.primaryNav : d.primaryNav;
  const primaryNav = primaryNavRaw
    .map((r) => ({ label: String(r?.label || '').trim(), href: String(r?.href || '').trim() }))
    .filter((r) => r.label && r.href);

  const legalRaw =
    Array.isArray(b.legalFooterLinks) && b.legalFooterLinks.length ? b.legalFooterLinks : d.legalFooterLinks;
  ...
  return {
    ...
    ctaHref: pick(b.ctaHref, landing.ctaHref, d.ctaHref),
    ...
    heroTitle: pick(landing.heroTitle, page.heroTitle, d.heroTitle),
    heroSubtitle: pick(landing.heroSubtitle, page.heroSubtitle, d.heroSubtitle),
