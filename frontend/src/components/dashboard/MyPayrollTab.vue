  ...
  >
    ...
    <PayrollHubSection section-id="pay_stubs" :open="true" :item-count="visiblePeriods.length" count-label="periods">
      <div v-if="!loading" class="periods">
        ...
        <template v-for="(p, idx) in visiblePeriods" :key="p.payroll_period_id">
          <button
            type="button"
            class="period-row"
          ...
          <!-- Breakdown drops directly under the selected pay period -->
          <div
            v-if="Number(expandedId) === Number(p.payroll_period_id) && expanded"
            class="details pay-stub-breakdown pay-stub-breakdown--inline"
            :ref="setBreakdownEl"
          >
        <div class="pay-stub-breakdown__header">
          <strong class="pay-stub-breakdown__title">
            ...
            Pay Breakdown — {{ fmtDateRange(expanded.period_start, expanded.period_end) }}
          </strong>
          <button type="button" class="pay-stub-breakdown__close" @click.stop="expandedId = null" aria-label="Close breakdown">×</button>
        </div>
        <div>
        <div
        ...
        <div v-else>
          ...
          <div class="hint" style="margin-top: 10px;">
            <template v-if="ptoAccount?.employment_type === 'fee_for_service'">
              Policy reminders: Sick Leave accrues at {{ fmtNum(ptoPolicy?.sickFfsMultiplier ?? 0.04) }} hours per 1 credit/hour of paid time after payroll is posted.
            </template>
            ...
            <template v-else>
              Policy reminders: Sick Leave accrues at {{ fmtNum(ptoPolicy?.sickHourlyMultiplier ?? 0.034) }} hours per 1 worked hour (direct + indirect + other paid time) after payroll is posted. Hourly staff do not earn Training PTO.
            </template>
...
const parseYmd = (d) => {
  if (!d) return null;
  // Accept 'YYYY-MM-DD' or ISO strings. Use local noon so UTC-midnight
  // parsing does not shift calendar dates west of UTC (e.g. Jul 4 → Jul 3).
  const s = String(d);
...
const toggle = async (id) => {
  ...
  const next = (Number.isFinite(cur) && cur === nid) ? null : nid;
  expandedId.value = next;
  if (next != null) {
    await nextTick();
    breakdownEl.value?.scrollIntoView?.({ behavior: 'smooth', block: 'nearest' });
  }
};
...
.warn-box {
  border: 1px solid var(--app-line, #ffe58f);
  background: var(--app-tint-amber, #fffbe6);
  border-radius: 10px;
  padding: 10px 12px;
}
.current-unpaid-notes {
  border: 1px solid var(--app-line, #ffd8a8);
  background: var(--app-tint-amber, #fff4e6);
}
.current-unpaid-notes strong {
  color: var(--app-text-red, #b45309);
}
.old-notes-alert {
  border: 1px solid var(--app-line, #ffb5b5);
  background: var(--app-surface-muted, #ffecec);
}
.old-notes-alert strong {
  color: var(--app-text-red, #b42318);
  font-weight: 800;
}
.clickable {
  cursor: pointer;
