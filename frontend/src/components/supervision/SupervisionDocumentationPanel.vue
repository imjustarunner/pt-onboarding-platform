<template>
  <section v-if="visible" class="documentation-panel" aria-label="Documentation oversight">
    ...
    <template v-else-if="policy">
      <p><strong>Responsible note signer:</strong> {{ signerName }}. A separate clinical supervisor can review cases and plans without cosigning notes.</p>
      <p>Supervisor assignments do not grant access to claims, payments, or financial reports.</p>
      ...
      <SupervisionCaseOverview :agency-id="agencyId" :provider-id="providerId" :can-review="canReview" @view-notes="viewCaseNotes" @open-document="openFromCase" />
      <details :open="queueOpen">
        <summary>Clinical document review queue</summary>
        <button :disabled="busy" @click="loadDocuments">Refresh documents</button>
        <p>Each page shows up to 100 signed notes and 100 active/final treatment plans. Clinical review and cosign are recorded separately.</p>
        ...
        <p v-if="documentClientId">Showing client #{{ documentClientId }} <button :disabled="busy" @click="viewCaseNotes(null)">All clients</button></p>
        <button :disabled="busy || documentPage === 1" @click="changeDocumentPage(-1)">Previous documents</button><span>Page {{ documentPage }}</span><button :disabled="busy || !hasMoreDocuments" @click="changeDocumentPage(1)">More documents</button>
        ...
        <section v-if="opened" class="document-review">
          ...
          <form v-if="canReview" @submit.prevent="recordReview">
            ...
            <button v-if="canAttest && opened.type === 'note' && !opened.cosignedAt" type="button" :disabled="busy || !reviewAttested || outcome !== 'approved'" @click="cosign">{{ opened.amendmentSignoffRequired ? 'Sign off on note and all amendments' : 'Cosign this note' }}</button>
      ...
      <details @toggle="onTimeToggle">
        ...
        <form v-if="canReview" @submit.prevent="saveTime">
          <label>Activity<select v-model="time.activityType"><option value="documentation_review">Documentation review</option><option v-if="canAttest" value="rendering_provider_oversight">Rendering provider oversight (RPO)</option></select></label>
...
const policy=ref(null),canManage=ref(false),canAttest=ref(false),canReview=ref(false),noteTypes=ref([]),visible=ref(true),loading=ref(false),busy=ref(false),error=ref(''),notice=ref(''),policyReason=ref('');
const signerName=computed(()=>policy.value?.supervisors?.find(s=>s.id===policy.value.supervisorUserId)?.name || (policy.value?.supervisorUserId?`Supervisor #${policy.value.supervisorUserId}`:'No supervisor assigned'));
const documentPage=ref(1),hasMoreDocuments=ref(false),documentClientId=ref(null),queueOpen=ref(false);
...
async function fetchDocuments(g=generation){const {data}=await api.get(`${base()}/document-reviews`,{params:{...params(),page:documentPage.value,clientId:documentClientId.value || undefined}});if(active&&g===generation){documents.value=data.documents||[];hasMoreDocuments.value=data.hasMore===true;}}
const loadDocuments=()=>run(fetchDocuments);
const viewCaseNotes=clientId=>{documentClientId.value=clientId;documentPage.value=1;queueOpen.value=true;loadDocuments();};
const changeDocumentPage=delta=>{documentPage.value+=delta;loadDocuments();};
const openFromCase=doc=>{queueOpen.value=true;openDocument(doc);};
