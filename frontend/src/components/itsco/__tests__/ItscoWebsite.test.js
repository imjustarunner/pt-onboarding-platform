...
describe('ITSCO directory and real support',()=>{
 ...
 it('persists a real support ticket and reports success only after confirmation',async()=>{api.post.mockResolvedValue({data:{ok:true,ticketId:42}});const w=mount(Support,{global});await w.find('input').setValue('Example Visitor');await w.find('input[type=email]').setValue('visitor@example.test');await w.find('textarea').setValue('A synthetic support question');await w.find('form').trigger('submit');await flushPromises();expect(api.post).toHaveBeenCalledWith('/public/school-referral/itsco/support-tickets',expect.objectContaining({name:'Example Visitor',message:'A synthetic support question'}),expect.anything());expect(w.text()).toContain('#42');w.unmount();});
 it('keeps the support message on failure',async()=>{api.post.mockRejectedValue(new Error('Unable to send'));const w=mount(Support,{global});await w.find('textarea').setValue('Keep this message');await w.find('form').trigger('submit');await flushPromises();expect(w.find('[role=alert]').text()).toContain('Unable to send');expect(w.find('textarea').element.value).toBe('Keep this message');expect(w.text()).not.toContain('Your message has been sent.');w.unmount();});
});
