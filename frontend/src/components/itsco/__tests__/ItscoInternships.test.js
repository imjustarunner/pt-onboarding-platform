...
const mounts=[];
async function render(section='internships'){const w=mount(Internships,{props:{section,data},global:{stubs:{RouterLink:{props:['to'],template:'<a :href="to"><slot/></a>'},PublicProviderProfileEditor:true,PublicWebsiteContactForm:true}}});mounts.push(w);await flushPromises();return w;}
afterEach(()=>{mounts.splice(0).forEach(w=>w.unmount());vi.clearAllMocks();});
describe('ITSCO internship recruitment',()=>{
 it('publishes paid placements in both cities and the dedicated Rachel inquiry form',async()=>{api.get.mockResolvedValue({data:{jobs}});const w=await render();expect(w.text()).toContain('Positions Available');expect(w.text()).toContain('3–5 positions');expect(w.text()).toContain('2–3 positions');expect(w.text()).toContain('$24–$30 per hour of direct service');expect(w.text()).toContain('Fall, Spring, or Summer');expect(w.find('#contact-rachel public-website-contact-form-stub').attributes('internshipinquiry')).toBe('true');});

 it('links directly to the published internship application and explains supervised phases',async()=>{api.get.mockResolvedValue({data:{jobs}});const w=await render();expect(w.text()).toContain('01 · Practicum');expect(w.text()).toContain('02 · Internship');expect(w.find('a[href$="/intake/real-intern-application"]').text()).toContain('Practicum/Internship application');expect(w.find('a[href$="/intake/provider"]').exists()).toBe(false);expect(w.text()).toContain('at least once a week');});
