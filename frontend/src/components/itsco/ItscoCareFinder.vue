...
function findCare(){router.push({path:'/p/itsco/providers',query:{care:care.value||undefined,setting:location.value.startsWith('virtual:')?'virtual':'all',city:location.value&&!location.value.startsWith('virtual:')?location.value:undefined,state:location.value.startsWith('virtual:')?location.value.split(':')[1]:undefined,insurance:insurance.value||undefined}});}
</script>
