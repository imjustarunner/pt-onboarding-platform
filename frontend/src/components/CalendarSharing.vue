<template>
  <section class="calendar-sharing">
    ...
    <p>{{ family?'Create a shared Google calendar for your household, or subscribe from any calendar app. Linked parents are added automatically.':'Keep a separate work calendar in your personal Google, Apple or Outlook account. SSO is not required. Sessions show their type and client initials; meetings include your personal join links (sign in with your invited account).' }}</p>
    <p v-if="!family">Enable sharing here to see new app meetings in your external calendar. Invitations and reminders come from the branded app email, not Google calendar invitation emails.</p>
    <p>Manage app-created events in this app. Google copies refresh about every five minutes. Subscription refresh timing is controlled by your calendar app.</p><p v-if="family">Events created directly in this shared Google calendar also appear in Family. To include events from your primary Google calendar, select it under “Bring in a Google calendar.”</p>
