...
<style scoped>.policy-fields { display:grid; grid-template-columns:repeat(auto-fit,minmax(210px,1fr)); gap:14px; } label { display:grid; gap:6px; font-size:14px; } input,select { box-sizing:border-box; width:100%; padding:10px; border:1px solid #94a3b8; border-radius:7px; background:var(--bg-card); color:var(--text-primary); } .self-subscriber{grid-column:1/-1;margin:0;padding:10px;border-radius:7px;background:var(--bg-secondary,#eef6fa);font-size:14px}.check { display:flex; align-items:center; } .check input { width:auto; }</style>
