...
<style scoped>.policy-fields { display:grid; grid-template-columns:repeat(auto-fit,minmax(210px,1fr)); gap:14px; } label { display:grid; gap:6px; font-size:14px; } input,select { width:100%; padding:10px; border:1px solid #94a3b8; border-radius:7px; background:var(--bg-card); color:var(--text-primary); } .check { display:flex; align-items:center; } .check input { width:auto; }</style>
