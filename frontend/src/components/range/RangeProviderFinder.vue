 ...
 <section class="range-finder" aria-label="Find providers">
  <div class="range-finder-tools">
   ...
   <template v-if="moreFilters">
    ...
    <ProviderTimeFilters :model-value="filters" time-zone="America/Denver" @update:model-value="setTimeFilters"/>
  ...
  <template v-else>
   ...
   <div class="range-network-grid">
    <article v-for="p in visible" :key="key(p)" class="range-provider-card" :data-analytics-id="`provider-${p.id}`" :data-analytics-label="p.name">
     ...
     <p v-if="p.bio" class="range-network-bio">{{p.bio}}</p><div class="range-tags"><span v-for="tag in matchingProviderTags(p,filters.search,filters.specialty,4)" :key="tag">{{tag}}</span></div>
...
const key=p=>`${p.agencyId}-${p.id}-${p.service}:${filters.value.day}:${filters.value.timeFrom}:${filters.value.timeTo}`,label=v=>services.find(s=>s[0]===v)?.[1]||v;
...
function setTimeFilters(v){for(const key of ['day','timeFrom','timeTo'])filters.value[key]=v[key]||'';}
const activeFilterCount=computed(()=>['day','timeFrom','timeTo','age','specialty','insurance','gender','care','school','state','agency','accepting','openings'].filter(k=>filters.value[k]).length);
const candidates=computed(()=>rows.value.filter(p=>{
 ...
 (!f.state||(f.setting==='virtual'?publicVirtualStates(p).includes(normalizeServiceState(f.state)):(p.locations||[]).some(o=>normalizeServiceState(o.state)===normalizeServiceState(f.state))||p.location?.toLowerCase().endsWith(f.state.toLowerCase())))&&
...
const filtered=computed(()=>candidates.value.filter(p=>{
 ...
 return (!hasTimePreference(f)||(known&&slots(p).length>0))&&(!f.accepting||status(p)===(f.accepting==='yes'?'accepting':f.accepting))&&(!f.openings||(known&&(f.openings==='yes'?hasOpenings(p):!hasOpenings(p))));
...
async function pump(){if(props.preview||disposed)return;while(running<3){const p=candidates.value.find(p=>!calendars.value[key(p)]);if(!p)break;const k=key(p);calendars.value[k]={loading:true};running++;
 api.get(`/public/mental-range/providers/${p.agencyId}/${p.id}/availability`,{params:{service:p.service,format:'ALL',day:filters.value.day||undefined,timeFrom:filters.value.timeFrom||undefined,timeTo:filters.value.timeTo||undefined},skipAuthRedirect:true,skipGlobalLoading:true,timeout:60000}).then(({data})=>{if(!disposed)calendars.value[k]=data;}).catch(()=>{if(!disposed)calendars.value[k]={error:true};}).finally(()=>{running--;pump();});}}
watch(candidates,pump);
watch(()=>[filters.value.day,filters.value.timeFrom,filters.value.timeTo],pump);
