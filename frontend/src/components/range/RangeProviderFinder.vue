<template>
 <section class="range-finder" aria-label="Find providers">
  ...
  <template v-else>
   <div class="range-results-heading"><h2>{{grouped.length}} provider{{grouped.length===1?'':'s'}} found</h2><label>Sort by<select v-model="sort"><option value="name">Name A–Z</option><option value="accepting">Accepting first</option></select></label></div>
   <div class="range-directory-layout">
    <div>
     ...
     <article v-for="p in visible" :key="p.id" class="range-provider-card">
      ...
      <details class="range-provider-details"><summary>View profile & availability <span>↗</span></summary>
       <section v-for="a in p.affiliations" :key="key(a)" class="range-affiliation">
        ...
        <div class="range-actions"><label>Session format<select v-model="formats[key(a)]"><option value="IN_PERSON">In person</option><option value="VIRTUAL">Virtual</option></select></label><button type="button" class="range-outline" :disabled="openings[key(a)]?.loading" @click="check(a)">{{openings[key(a)]?.loading?'Checking…':'Check current openings'}}</button><a class="range-button" :href="`${a.bookingUrl}&programType=${formats[key(a)]||'IN_PERSON'}`" @click="guard">Continue with this provider →</a></div>
...
const rows=ref([]),loading=ref(true),error=ref(''),search=ref(''),service=ref(''),agency=ref(String(route.query.agency||'')),location=ref(''),age=ref(''),insurance=ref(''),accepting=ref(''),sort=ref('name'),pageNumber=ref(1),formats=ref({}),openings=ref({}),notice=ref('');
