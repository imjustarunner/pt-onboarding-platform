<template>
  <Teleport to="body">
    <div v-if="open" class="drawer-backdrop" @click.self="$emit('close')">
      <aside class="settings-drawer" role="dialog" aria-modal="true" aria-label="Notification settings">
        <header>
...
.settings-drawer { width:min(1080px,98vw); height:100%; background:var(--background,#fff); box-shadow:-18px 0 50px rgba(15,23,42,.2); display:flex; flex-direction:column; }
