    ...
    <div class="rlm-section">
      ...
      <div v-else class="rlm-list">
        <div v-for="a in awards" :key="a.id" class="rlm-row">
          <div class="rlm-row-main">
            <span class="rlm-award-icon">{{ a.icon }}</span>
            <div>
          ...
          <div class="rlm-field-row">
            ...
            <div class="rlm-field rlm-field--narrow">
              ...
              <div class="rlm-icon-cell" @click="showIconPicker = !showIconPicker">
                <span class="rlm-icon-preview">{{ awardForm.icon }}</span>
...
watch(() => props.clubId, (id) => {
  if (id) { loadGroups(); loadAwards(); }
  else { groups.value = []; awards.value = []; }
}, { immediate: false });

onMounted(() => { if (props.clubId) { loadGroups(); loadAwards(); } });

// expose for parent to refresh after "save to library"
defineExpose({ loadAwards, loadGroups, awards, groups });
...
function openAwardModal(a = null) {
  ...
  awardForm.value = a
    ? { label: a.label, icon: a.icon || '🏆', period: a.period, monthEndDay: a.monthEndDay || 'last', metric: a.metric, aggregation: a.aggregation, activityType: a.activityType || '', groupFilter: a.groupFilter || '' }
    : defaultAwardForm();
...
.rlm-award-icon { font-size: 20px; flex-shrink: 0; }
