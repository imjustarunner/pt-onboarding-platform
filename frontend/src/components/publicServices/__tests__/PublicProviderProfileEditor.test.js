...
describe('public profile editing',()=>{
 ...
 it('saves actual profile data through the protected API and refreshes the public view',async()=>{const w=render();await flushPromises();await w.find('button').trigger('click');await w.find('textarea').setValue('Updated public biography');await w.find('input[aria-label="Thursday afternoons"]').setValue(true);await w.find('form').trigger('submit');await flushPromises();expect(api.put).toHaveBeenCalledWith('/users/9/provider-public-profile',expect.objectContaining({agencyId:1,publicBlurb:'Updated public biography',identity:{firstName:'Test',lastName:'Provider',title:'Counselor'},insurances:['Existing insurer'],details:expect.objectContaining({languages:['English'],typicalAvailability:['Thursday afternoons']}),acceptingNewClientsOverride:null}));expect(w.emitted('saved')).toHaveLength(1);w.unmount();});
