...
describe('company setup scope',()=>{
 it('opens business details directly for a coaching company in the selected tenant',async()=>{api.get.mockResolvedValue({data:{id:1,name:'Example Coaching',organization_type:'life_coach',slug:'example'}});const {w}=await setup();expect(w.text()).toContain('Example Coaching');expect(w.find('.business-editor').exists()).toBe(true);expect(w.findComponent({name:'AgencyManagement'}).props('embeddedOrgId')).toBe(1);w.unmount();});
