  ...
  <div class="payer-panel">
    <div class="payer-panel-header">
      <div>
        ...
        <p class="muted">Track submitted / returned / effective dates, documents, and notes for each payer.</p>
      </div>
    ...
    <div class="provider-notes-block">
      <label>Provider credentialing notes</label>
      <textarea
        v-model="providerNotesDraft"
        rows="2"
        placeholder="Notes that apply to this provider across all payers…"
        :disabled="savingProviderNotes"
      />
      <div class="notes-actions">
        <button
          class="btn btn-secondary btn-sm"
          type="button"
          :disabled="savingProviderNotes || providerNotesDraft === providerNotesSaved"
          @click="saveProviderNotes"
        >
          {{ savingProviderNotes ? 'Saving…' : 'Save provider notes' }}
        </button>
        <span v-if="providerNotesMsg" class="muted">{{ providerNotesMsg }}</span>
      </div>
    </div>
    ...
    <div v-else class="payer-cards">
      <div v-for="row in rows" :key="row.recordId || row.definitionId" class="payer-card">
        ...
        <div class="optional-toggles">
          ...
          >
            {{ row.showProviderId ? '−' : '+' }} Provider ID
          </button>
          <button
            type="button"
            class="plus-btn"
        ...
        <div v-if="row.showLogin" class="form-grid optional-field">
          ...
          <div v-if="row.hasUserCredentials" class="form-group reveal-group">
...
const emit = defineEmits(['changed', 'provider-notes-saved']);
...
const revealed = ref({});
const providerNotesDraft = ref('');
const providerNotesSaved = ref('');
const savingProviderNotes = ref(false);
const providerNotesMsg = ref('');
  ...
  return {
    ...
    contractUrl: cred?.contract_url
      ...
      : cred?.contract_path
        ...
        : null,
    showProviderId: !!pin,
    showLogin: !!cred?.has_user_credentials,
    saving: false,
...
const load = async () => {
  ...
  try {
    const [credRes, payerRes] = await Promise.all([
      api.get(`/agencies/${props.agencyId}/credentialing/users/${props.userId}`),
    ...
    rows.value = (payers.value || [])
      ...
      .filter(Boolean);
    // Include orphaned credential rows if definition was renamed/removed from list order
...
const createCredential = async () => {
  ...
  error.value = '';
  try {
    await api.post(`/agencies/${props.agencyId}/credentialing/user-insurance`, {
      userId: props.userId,
...
const removeCredential = async (row) => {
  if (!confirm(`Remove ${row.name} credential for this provider?`)) return;
  row.saving = true;
  row.error = '';
  try {
    await api.delete(`/agencies/${props.agencyId}/credentialing/user-insurance`, {
      data: {
        userId: props.userId,
        insuranceCredentialingDefinitionId: row.definitionId
      }
...
const uploadDoc = async (row, docType, event) => {
  ...
  try {
    ...
    const res = await api.post(
      `/agencies/${props.agencyId}/credentialing/user-insurance/${row.recordId}/documents/${docType}`,
      form,
...
const toggleReveal = async (row) => {
  ...
  try {
    ...
    const user = res.data?.value || res.data?.username || '';
    let pass = '';
    try {
      ...
      pass = passRes.data?.value || passRes.data?.password || '';
    } catch {
      pass = '';
    }
    revealed.value = {
...
const saveProviderNotes = async () => {
  savingProviderNotes.value = true;
  providerNotesMsg.value = '';
  try {
    await api.patch(`/agencies/${props.agencyId}/credentialing/providers`, {
      updates: [
        {
          userId: props.userId,
          fieldKey: 'credentialing_provider_notes',
          value: providerNotesDraft.value || null
        }
      ]
    });
    providerNotesSaved.value = providerNotesDraft.value;
    providerNotesMsg.value = 'Saved.';
    emit('provider-notes-saved', providerNotesDraft.value);
  } catch (e) {
    providerNotesMsg.value = e.response?.data?.error?.message || 'Failed to save notes';
  } finally {
    savingProviderNotes.value = false;
  }
};
...
watch(
  () => props.providerNotes,
  (v) => {
    providerNotesDraft.value = String(v || '');
    providerNotesSaved.value = String(v || '');
  },
  { immediate: true }
);
</script>
