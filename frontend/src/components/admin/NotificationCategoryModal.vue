...
const categories = computed(() => {
  const categoryTypes = [
    ...
    { type: 'company_event_registration_submitted', label: 'New Event Registration' },
    { type: 'support_ticket_created', label: 'Support Tickets' },
    { type: 'support_ticket_forwarded_to_provider', label: 'Forwarded client message (support)' },
...
const fetchCategoryCounts = async () => {
  try {
    ...
    const counts = {
      ...
      company_event_registration_submitted: 0,
      support_ticket_created: 0,
      support_ticket_forwarded_to_provider: 0,
