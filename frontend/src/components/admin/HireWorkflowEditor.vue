<template>
  <div class="workflow-editor">
    ...
    <div class="add-row"><select v-model="catalogKey" aria-label="Add a portal step"><option value="">Choose an item to add…</option><option v-for="item in availableCatalog" :key="item.id" :value="item.id">{{ item.title }}</option></select><button type="button" @click="addCatalog" :disabled="!catalogKey">Add item</button><button type="button" @click="addCustom">Add custom step</button></div>
...
function addCustom() { patch({ resources: [...(props.modelValue.resources || []), { id: crypto.randomUUID(), title: '', phase: props.phase || 'onboarding', kind: 'link', required: true, url: '' }] }); }
</script>
<style scoped>
