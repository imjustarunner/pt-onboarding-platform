<template>
  <div class="workflow-editor">
    ...
    <div class="add-row"><button v-for="kind in ['video', 'link', 'meeting']" :key="kind" type="button" @click="addCustom(kind)">Add {{ kind }}</button></div>
    <div class="add-row"><select v-model="catalogKey" aria-label="Add a portal step"><option value="">Choose an item to add…</option><option v-for="item in availableCatalog" :key="item.id" :value="item.id">{{ item.title }}</option></select><button type="button" @click="addCatalog" :disabled="!catalogKey">Add item</button><button type="button" @click="addCustom('link')">Add custom step</button></div>
    <div v-if="!resources.length" class="empty">{{ phase === 'onboarding' ? 'No additional steps. The selected onboarding collection and built-in onboarding steps will be included.' : 'No extra steps configured. Background authorization, job description, employment agreement, work email, pre-employment information, headshot and handbook are built in.' }}</div>
    <article v-for="(item, index) in resources" :key="item.id" class="resource-editor">
      ...
      <div class="editor-grid"><label>Title<input :value="item.title" @input="change(item.id, { title: $event.target.value })" /></label><label v-if="!phase">Process<select :value="item.phase" @change="change(item.id, { phase: $event.target.value })"><option value="pre_hire">Pre-hire</option><option value="onboarding">Onboarding</option></select></label><label>Step type<select :value="item.kind" @change="change(item.id, { kind: $event.target.value })"><option value="document">Document template · fill, review and sign</option><option value="acknowledgement">Linked document · signed acknowledgement</option><option value="upload">Download blank form · upload completed copy</option><option value="video">Embedded video</option><option value="link">Training / resource link</option><option value="meeting">Meeting scheduler</option></select></label><label v-if="item.kind === 'document'">Document template<select :value="item.templateId" @change="change(item.id, { templateId: Number($event.target.value) || null })"><option value="">Choose a template</option><option v-for="t in templates" :key="t.id" :value="t.id">{{ t.name }}</option></select><small>Existing PDF fields are supported. Candidates can place their own text and signature on flat PDFs.</small><a v-if="editorUrl" :href="templateEditorUrl(item.templateId)" target="_blank" rel="noopener">View or edit document template ↗</a></label><label v-else>{{ item.kind === 'meeting' ? 'Scheduling link' : 'Viewer, video or blank form link' }}<input :value="item.url" type="url" placeholder="https://…" @input="change(item.id, { url: $event.target.value })" /></label></div>
...
const props = defineProps({ modelValue: { type: Object, default: () => ({}) }, templates: { type: Array, default: () => [] }, heading: { type: String, default: 'Portal steps' }, branding: Boolean, supervisorSettings: Boolean, phase: String, editorUrl: String });
const emit = defineEmits(['update:modelValue']);
const resources = computed(() => (props.modelValue.resources || []).filter(r => !props.phase || r.phase === props.phase));
...
function templateEditorUrl(id) {
  const [path, query] = (props.editorUrl || '').split('?');
  return `${path}${id ? '/' + id + '/edit' : ''}${query ? '?' + query : ''}`;
}
function patch(values) { emit('update:modelValue', { ...props.modelValue, ...values }); }
function change(id, values) { patch({ resources: (props.modelValue.resources || []).map(r => r.id === id ? { ...r, ...values } : r) }); }
function remove(id) { patch({ resources: (props.modelValue.resources || []).filter(r => r.id !== id), excludedResourceIds: [...new Set([...(props.modelValue.excludedResourceIds || []), id])] }); }
function addCatalog() { const item = catalog.find(r => r.id === catalogKey.value); if (item && !resources.value.some(r => r.id === item.id)) patch({ resources: [...(props.modelValue.resources || []), { ...item }], excludedResourceIds: (props.modelValue.excludedResourceIds || []).filter(id => id !== item.id) }); catalogKey.value = ''; }
function addCustom(kind = 'link') { patch({ resources: [...(props.modelValue.resources || []), { id: crypto.randomUUID(), title: '', phase: props.phase || 'onboarding', kind, required: true, url: '' }] }); }
</script>
<style scoped>
