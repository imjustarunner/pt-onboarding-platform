<template>
  ...
  <div v-if="showTimeOvertimeModal" class="modal-backdrop" @click.self="closeTimeOvertimeModal">
    <div class="modal" style="width: min(720px, 100%);">
      ...
      <div class="field" style="margin-top: 10px;">
        <label>Hours worked each day (Mon–Sun)</label>
        <div class="hint" style="margin-bottom: 8px;">Overtime = over 12 hrs/day or over 40 hrs/week.</div>
      ...
      <div class="actions" style="margin-top: 12px; justify-content: flex-end;">
        <button class="btn btn-primary" @click="submitTimeOvertime" :disabled="submittingTimeClaim || overtimeWeekTotal <= 0">
          {{ submittingTimeClaim ? 'Submitting…' : 'Submit for approval' }}
        </button>
      </div>
    </div>
...
const submitTimeOvertime = async () => {
  ...
  if (!datesAndHoursStr.trim()) {
    submitTimeClaimError.value = 'Please enter a valid week date and hours for each day.';
    return;
  }
  await submitTimeClaim({
