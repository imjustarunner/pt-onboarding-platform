<template>
  <div class="koa-overlay" @click.self="$emit('close')">
    <div class="koa-panel" role="dialog" aria-modal="true" aria-label="Office Availability">
      ...
      <div v-if="!selectedRoom" class="koa-body">
        <p class="koa-body__subtitle">
          The following offices are available right now. Select one to reserve it for the rest of the day.
        </p>
        ...
        <div v-else-if="rooms.length === 0" class="koa-empty">
          No offices are currently available. Please see the front desk for assistance.
        </div>
        ...
        <div v-else class="koa-rooms-grid">
          ...
          >
            ...
            <div v-if="room.room_number" class="koa-room-card__number">Office {{ room.room_number }}</div>
            <div class="koa-room-card__avail">Available Now</div>
          </button>
        </div>

        <button class="koa-btn koa-btn--secondary" style="margin-top: 20px;" @click="$emit('close')">
          Close
        </button>
      </div>

      <!-- PIN entry for selected room -->
      <div v-else-if="!reserved" class="koa-body">
        <button class="koa-back-btn" @click="selectedRoom = null">
        ...
        <div class="koa-room-selected">
          ...
          <div>
            ...
            <div v-if="selectedRoom.room_number" class="koa-room-selected__number">Office {{ selectedRoom.room_number }}</div>
          </div>
        </div>

        <p class="koa-body__subtitle" style="margin-bottom: 20px;">
          Enter your 4-digit kiosk PIN to reserve this office for the rest of the day.
        </p>
        ...
        <div class="koa-pin-display" aria-label="PIN entered" aria-live="polite">
          <span
            ...
            :key="i"
            class="koa-pin-dot"
            :class="{ 'koa-pin-dot--filled': pin.length >= i }"
          />
        </div>
      ...
      <div v-else class="koa-body koa-body--center">
        ...
        <p class="koa-body__subtitle">
          ...
          has been booked in <strong>{{ reserved.roomName }}</strong>
          until end of day.
        </p>
        <button class="koa-btn koa-btn--primary" style="margin-top: 24px;" @click="$emit('close')">
          Done
        </button>
      </div>
    </div>
...
async function submitReservation() {
  ...
  try {
    ...
    if (e?.response?.status === 401) {
      ...
      pinError.value = msg || 'Reservation failed. Please see the front desk.';
    }
    pin.value = '';
  } finally {
...
.koa-room-selected__name {
  font-size: 17px;
  font-weight: 700;
  color: #1a2f3a;
}
.koa-room-selected__number {
  font-size: 13px;
  color: #6b8494;
}
