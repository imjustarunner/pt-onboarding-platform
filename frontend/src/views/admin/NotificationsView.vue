...
const getNotificationNavigationPath = async (notification) => {
  ...
  if (notification.type === 'task_overdue' && notification.related_entity_type === 'task' && notification.related_entity_id) {
    ...
    try {
      ...
      if (slug) return `/${slug}/messages?threadId=${encodeURIComponent(String(threadId))}&agencyId=${encodeURIComponent(String(agencyId))}`;
      return `/messages?threadId=${encodeURIComponent(String(threadId))}&agencyId=${encodeURIComponent(String(agencyId))}`;
    } catch {
      const agencyId = notification.agency_id || '';
      return `/messages?threadId=${encodeURIComponent(String(notification.related_entity_id))}&agencyId=${encodeURIComponent(String(agencyId))}`;
    }
