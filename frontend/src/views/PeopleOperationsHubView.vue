<template>
  <div class="wo-hub so-hub po-hub">
    ...
    <header class="hub-header" data-tour="people-ops-hub-header">
      <div class="hub-brand">
        ...
        <div>
          <h1 class="hub-title" data-tour="people-ops-hub-title">People Operations</h1>
          <p class="hub-subtitle" data-tour="people-ops-hub-subtitle">
            Hiring, careers, onboarding, and development — organized in one place.
          </p>
        </div>
      </div>
    ...
    <div class="hub-layout" data-tour="people-ops-hub-grid">
      <div class="hub-stage-wrap">
        ...
        >
          ...
          <div class="hub-center-wrap">
            <button
              ...
              :aria-label="activeSectionId ? `Return to all areas` : 'People Operations hub'"
          ...
          >
            ...
            <template v-else>
              ...
              <div
                v-else
                class="hub-satellite hub-satellite--card hub-satellite--soon"
              ...
              >
                ...
                <div class="hub-satellite-title">{{ node.title }}</div>
                <p class="hub-satellite-desc">{{ node.shortDesc }}</p>
                <span class="hub-satellite-pill">Soon</span>
              </div>
              <span
                v-if="node.count > 0"
                class="hub-satellite-badge hub-satellite-badge-pulse"
              ...
              >
                ...
                <ul class="hub-preview-list">
                  <li v-for="card in node.cards" :key="card.id">
                    ...
                    <span v-else class="hub-preview-link hub-preview-link--soon">
                      <span class="hub-preview-dot" />
                      {{ card.title }}
                      <span class="hub-preview-count">Soon</span>
                    </span>
                  </li>
                </ul>
              </div>
        ...
        <p class="hub-stage-tip">
          ...
          <template v-else>
            Tip: Hover a category to preview what's inside. Click to explore hiring, onboarding, and development tools.
          </template>
        </p>
      </div>
      ...
      <aside class="hub-sidebar">
        <section class="hub-sidebar-panel">
          ...
          <ul class="hub-glance-list">
            <li v-if="canManageHiring && hasHiringFeature">
              <span class="glance-dot school" />
              <span>Applications</span>
              <strong>{{ applicationsPending }}</strong>
              <em>Open</em>
            </li>
            <li v-if="canManageHiring && hasHiringFeature">
        ...
        <section class="hub-sidebar-panel">
          ...
          <div class="hub-quick-links">
            ...
            <router-link
              v-if="canManageHiring && hasHiringFeature"
              :to="orgTo('/admin/onboarding')"
              class="hub-quick-link"
            >
              <span>Onboarding · Send invite</span>
              <span v-if="onboardingPending > 0" class="hub-quick-badge">{{ onboardingPending }}</span>
            </router-link>
            <router-link
              v-if="canManageHiring && hasHiringFeature"
              :to="orgTo('/admin/careers/jobs')"
              class="hub-quick-link"
            >Job postings</router-link>
            <router-link
              v-if="canManageHiring && (hasHiringFeature || hasPeopleOpsFeature)"
              :to="orgTo('/admin/employee-relations')"
              class="hub-quick-link"
            >Employee Relations</router-link>
          </div>
        </section>
      </aside>
    </div>
      ...
      <section v-for="section in visibleSections" :key="section.id" class="hub-mobile-section">
        ...
        <div class="hub-mobile-grid">
          ...
          <div
            ...
            :key="card.id"
            class="hub-mobile-card hub-mobile-card--soon"
            :class="[`tone-${card.tone}`]"
          >
            <span class="hub-mobile-badge">Soon</span>
            <div class="hub-mobile-icon" v-html="card.icon" />
            <div class="hub-mobile-title">{{ card.title }}</div>
            <p>{{ card.desc }}</p>
          </div>
...
const hasCapability = (key) => !!(user.value?.capabilities || {})[key];
const canManageHiring = computed(() => hasCapability('canManageHiring'));
const canViewTraining = computed(() => hasCapability('canViewTraining'));
const canSignDocuments = computed(() => hasCapability('canSignDocuments'));

const hubNavCtx = computed(() => workspaceNavContextFromStores({
...
const hubSwitcherLinks = computed(() => buildHubSwitcherLinks({
  ...hubNavCtx.value,
  currentSurface: 'people'
}));

const agencyId = computed(() => Number(agencyStore.currentAgency?.id || 0));
const canSeeHub = computed(() => canAccessPeopleOperationsHub(hubNavCtx.value));

const applicationsPending = ref(0);
const onboardingPending = ref(0);
const hubPendingTotal = computed(() => applicationsPending.value + onboardingPending.value);
...
const icon = {
  hub: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="3"/><path d="M22 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" stroke-linecap="round"/></svg>',
  careers: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2M12 12v4M8 12h8" stroke-linecap="round"/></svg>',
  ...
  hiring: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="3"/><path d="M19 8v6M22 11h-6" stroke-linecap="round"/></svg>',
  training: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"><path d="m12 3 9 5-9 5-9-5 9-5z"/><path d="M5 10v5c0 1.5 3 3 7 3s7-1.5 7-3v-5M12 13v8" stroke-linecap="round"/></svg>',
  progress: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"><path d="M3 3v18h18"/><path d="M7 14l4-4 4 3 5-6" stroke-linecap="round" stroke-linejoin="round"/></svg>',
  ...
  offboard: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><path d="M16 17l5-5-5-5M21 12H9" stroke-linecap="round" stroke-linejoin="round"/></svg>',
...
const allSections = computed(() => [
  {
    id: 'careers',
    label: 'Careers',
    desc: 'Public careers page and open roles.',
    tone: 'blue',
    icon: icon.careers,
    cards: [
      {
      ...
      },
      {
        ...
        shortDesc: 'Branding, copy, and page content.',
        desc: 'Configure the public careers page — branding, sections, and messaging.',
        cta: 'Open →',
        to: orgTo('/admin/careers/page'),
        tone: 'blue',
        icon: icon.page,
        tour: 'people-ops-career-page',
      ...
      },
      {
        id: 'job-postings',
        title: 'Job postings',
        shortDesc: 'Open roles and descriptions.',
        desc: 'Create and manage job postings shown on the careers page.',
        cta: 'Open →',
        to: orgTo('/admin/careers/jobs'),
        tone: 'indigo',
        icon: icon.careers,
        tour: 'people-ops-job-postings',
        show: canManageHiring.value && hasHiringFeature.value,
        count: 0
      }
    ].filter((c) => c.show)
  },
  {
    id: 'hiring',
    label: 'Hiring',
    desc: 'Dashboard, applicants, interviews, and pre-hire.',
    tone: 'amber',
    icon: icon.hiring,
    cards: [
      {
        ...
        shortDesc: 'Pipeline stats and interviews.',
        desc: 'Hiring overview with pipeline stats and upcoming interviews.',
        cta: 'Open →',
        to: orgTo('/admin/hiring'),
        tone: 'teal',
        icon: icon.people,
        tour: 'people-ops-hiring-dashboard',
        show: canManageHiring.value && hasHiringFeature.value,
        count: 0
      },
      {
        ...
        tone: 'amber',
        icon: icon.hiring,
        tour: 'people-ops-applicants',
        show: canManageHiring.value && hasHiringFeature.value,
        count: applicationsPending.value
      },
      {
        id: 'interview-hub',
        title: 'Interview Hub',
        shortDesc: 'Interview scheduling and feedback.',
        desc: 'Coordinate interviews and capture feedback.',
        cta: 'Open →',
        to: orgTo('/admin/interview-hub'),
        tone: 'orange',
        icon: icon.interview,
        tour: 'people-ops-interview-hub',
        show: canManageHiring.value && hasHiringFeature.value,
        count: 0
      },
      {
        id: 'pre-hire',
        title: 'Pre-Hire',
        shortDesc: 'Pre-hire checklists and tasks.',
        desc: 'Manage pre-hire packages before onboarding starts.',
        cta: 'Open →',
        to: orgTo('/admin/pre-hire'),
        tone: 'rose',
        icon: icon.onboard,
  ...
  },
  {
    ...
    desc: 'Training, progress, and documents.',
    tone: 'green',
    icon: icon.training,
    cards: [
      {
        ...
        shortDesc: 'Course builder and modules.',
        desc: 'Build and manage training courses and modules.',
        cta: 'Open →',
        to: orgTo('/admin/modules'),
        tone: 'green',
        icon: icon.training,
        tour: 'people-ops-modules',
        show: hasPeopleOpsFeature.value && isAdmin.value && actorRole.value !== 'clinical_practice_assistant' && canViewTraining.value,
        count: 0
      },
      {
        id: 'progress',
        title: 'Progress',
        shortDesc: 'Staff training completion.',
        desc: 'Track staff training completion and progress.',
        cta: 'Open →',
        to: orgTo('/admin/agency-progress'),
        tone: 'teal',
        icon: icon.progress,
        tour: 'people-ops-progress',
        show: hasPeopleOpsFeature.value && canViewTraining.value,
        count: 0
      },
      {
        id: 'documents',
        title: 'Documents',
        shortDesc: 'Agency documents to sign.',
        desc: 'View, assign, and manage agency documents.',
        cta: 'Open →',
  ...
  },
  {
    id: 'onboarding',
    label: 'Onboarding',
    desc: 'New-hire onboarding checklists.',
    tone: 'cyan',
    icon: icon.onboard,
    cards: [
      {
        id: 'onboarding-admin',
        title: 'Onboarding',
        shortDesc: 'Progress and portal invites.',
        desc: 'Track onboarding progress and resend portal / login invites.',
        cta: onboardingPending.value > 0 ? `Review ${onboardingPending.value} →` : 'Open →',
        to: orgTo('/admin/onboarding'),
        tone: 'cyan',
        icon: icon.onboard,
        tour: 'people-ops-onboarding',
        show: canManageHiring.value && hasHiringFeature.value,
        count: onboardingPending.value
      }
    ].filter((c) => c.show)
  },
  {
    id: 'offboarding',
    label: 'Offboarding',
    desc: 'Separation workflows (coming soon).',
    tone: 'orange',
    icon: icon.offboard,
    cards: [
      {
        id: 'offboarding-soon',
        title: 'Offboarding',
        shortDesc: 'Separation checklists coming soon.',
        desc: 'Offboarding workflows and checklists will live here.',
        cta: 'Soon',
        to: null,
        disabled: true,
        tone: 'orange',
  ...
  },
  {
    ...
    cards: [
      ...
      },
      {
        ...
        shortDesc: 'Attestations coming soon.',
        desc: 'Policy distribution and acknowledgements will live here.',
        cta: 'Soon',
        to: null,
        disabled: true,
        tone: 'indigo',
  ...
  },
  {
    ...
    desc: 'Engagement, retention, and milestones.',
    tone: 'rose',
    icon: icon.relations,
    cards: [
      {
        ...
        shortDesc: 'Service milestones and gifts.',
        desc: 'Track service anniversaries, gifts owed, and acknowledgements.',
        cta: 'Open →',
        to: orgTo('/admin/employee-relations'),
        tone: 'rose',
        icon: icon.relations,
        tour: 'people-ops-employee-relations',
        show: canManageHiring.value && (hasHiringFeature.value || hasPeopleOpsFeature.value),
        count: 0
      }
    ].filter((c) => c.show)
  }
...
const centerTitle = computed(() => (activeSection.value ? activeSection.value.label : 'People Operations'));
const centerDesc = computed(() =>
  activeSection.value
    ? activeSection.value.desc
    : 'Hiring, careers, onboarding, and development — connected.'
);
...
const loadPendingCounts = async () => {
  if (!canSeeHub.value || !canManageHiring.value || !hasHiringFeature.value) {
    applicationsPending.value = 0;
    onboardingPending.value = 0;
    return;
  }
  try {
    const params = agencyId.value ? { agencyId: agencyId.value } : undefined;
    ...
    applicationsPending.value = Number(
      stats.pendingReviews
      ?? stats.totalApplicants
      ?? stats.stageCounts?.totalActive
      ?? 0
    );
    const onboardList = Array.isArray(onboardRes?.data) ? onboardRes.data : (onboardRes?.data?.candidates || []);
    onboardingPending.value = onboardList.filter((e) => Number(e.progress_pct || 0) < 100).length;
  } catch {
    applicationsPending.value = 0;
    onboardingPending.value = 0;
  }
...
<style scoped>
.wo-hub {
  --hub-ink: color-mix(in srgb, var(--primary, #1f6b4a) 22%, var(--app-ink, #0f172a));
  --hub-muted: var(--app-muted, #64748b);
  --hub-line: var(--app-line, #e2e8f0);
  --hub-panel: var(--app-surface, #fff);
  --hub-bg: var(--app-surface-muted, #f8fafc);
  position: relative;
  width: 100%;
  max-width: none;
  margin: 0;
...
.hub-icon {
  ...
  background: color-mix(in srgb, var(--primary, #1f6b4a) 12%, var(--app-surface, #fff));
...
.hub-calendar-btn {
  ...
  border: 1px solid var(--hub-line); background: var(--app-surface, #fff);
...
.hub-alert {
  ...
  border: 1px solid var(--app-line, #fecaca); border-radius: 12px; background: var(--app-surface-muted, #fef2f2);
}
.hub-alert-copy strong { color: var(--app-text-red, #991b1b); font-size: 13px; font-weight: 700; }
.hub-alert-meta { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
.hub-pill {
  display: inline-flex; padding: 4px 10px; border-radius: 999px;
  background: var(--app-surface, #fff); border: 1px solid var(--app-line, #fecaca);
  color: var(--app-text-red, #991b1b); font-size: 12px; font-weight: 700;
}
.hub-pill-link {
  text-decoration: none;
  cursor: pointer;
...
.hub-center {
  ...
  aspect-ratio: 1;
  border-radius: 50%;
  border: 1px solid var(--app-line, #e9d5ff);
  background:
    radial-gradient(circle at 50% 30%, var(--app-surface-muted, #f8fafc) 0%, var(--app-surface, #ffffff) 58%);
  box-shadow:
...
.hub-center.tone-purple {
  border-color: var(--app-line, #cbd5e1);
  background: radial-gradient(circle at 50% 30%, var(--app-surface-muted, #f8fafc) 0%, var(--app-surface, #ffffff) 58%);
  box-shadow:
    0 0 0 8px rgba(255, 255, 255, 0.65),
...
.hub-center.tone-green {
  border-color: var(--app-line, #bbf7d0);
  background: radial-gradient(circle at 50% 30%, var(--app-surface-muted, #ecfdf5) 0%, var(--app-surface, #ffffff) 58%);
  box-shadow:
    0 0 0 8px rgba(255, 255, 255, 0.65),
...
.hub-center.tone-blue {
  border-color: var(--app-line, #bfdbfe);
  background: radial-gradient(circle at 50% 30%, var(--app-surface-muted, #eff6ff) 0%, var(--app-surface, #ffffff) 58%);
  box-shadow:
    0 0 0 8px rgba(255, 255, 255, 0.65),
    0 0 48px rgba(59, 130, 246, 0.2),
    0 20px 50px rgba(59, 130, 246, 0.1);
}
.hub-center.tone-root .hub-center-icon { background: var(--app-surface-muted, #ecfdf5); color: var(--app-text-green, #16a34a); }
.hub-center.tone-indigo {
  border-color: var(--app-line, #c7d2fe);
  background: radial-gradient(circle at 50% 30%, var(--app-surface-muted, #ecfeff) 0%, var(--app-surface, #ffffff) 58%);
  box-shadow:
    0 0 0 8px rgba(255, 255, 255, 0.65),
    0 0 48px rgba(14, 116, 144, 0.2),
    0 20px 50px rgba(14, 116, 144, 0.1);
...
.hub-center.tone-teal {
  ...
  background: radial-gradient(circle at 50% 30%, var(--app-surface-muted, #f0fdfa) 0%, var(--app-surface, #ffffff) 58%);
  box-shadow:
    0 0 0 8px rgba(255, 255, 255, 0.65),
...
.hub-center.tone-cyan {
  border-color: var(--app-line, #67e8f9);
  background: radial-gradient(circle at 50% 30%, var(--app-surface-muted, #ecfeff) 0%, var(--app-surface, #ffffff) 58%);
  box-shadow:
    0 0 0 8px rgba(255, 255, 255, 0.65),
...
.hub-center.tone-indigo .hub-center-icon { background: var(--app-surface-muted, #ecfeff); color: var(--app-text-blue, #0e7490); }
.hub-center.tone-teal .hub-center-icon { background: var(--app-surface-muted, #f0fdfa); color: var(--app-text-green, #0f766e); }
.hub-center.tone-cyan .hub-center-icon { background: var(--app-surface-muted, #ecfeff); color: var(--app-text-blue, #0e7490); }
.hub-center.tone-purple .hub-center-icon { background: var(--app-surface-muted, #f8fafc); color: var(--app-muted, #475569); }
.hub-center.tone-green .hub-center-icon { background: var(--app-surface-muted, #ecfdf5); color: var(--app-text-green, #16a34a); }
.hub-center.tone-blue .hub-center-icon { background: var(--app-surface-muted, #eff6ff); color: var(--app-text-blue, #2563eb); }
...
.hub-satellite.tone-purple {
  border-color: var(--app-line, #c4b5fd);
  background: radial-gradient(circle at 50% 28%, var(--app-surface-muted, #f8fafc) 0%, var(--app-surface, #ffffff) 65%);
  box-shadow:
...
.hub-satellite.tone-green {
  ...
  background: radial-gradient(circle at 50% 28%, var(--app-surface-muted, #ecfdf5) 0%, var(--app-surface, #ffffff) 65%);
  box-shadow:
...
.hub-satellite.tone-blue {
  border-color: var(--app-line, #93c5fd);
  background: radial-gradient(circle at 50% 28%, var(--app-surface-muted, #eff6ff) 0%, var(--app-surface, #ffffff) 65%);
  box-shadow:
...
.hub-satellite.tone-orange {
  border-color: var(--app-line, #fdba74);
  background: radial-gradient(circle at 50% 28%, var(--app-surface-muted, #fff7ed) 0%, var(--app-surface, #ffffff) 65%);
  box-shadow:
...
.hub-satellite.tone-indigo {
  border-color: var(--app-line, #a5b4fc);
  background: radial-gradient(circle at 50% 28%, var(--app-surface-muted, #ecfeff) 0%, var(--app-surface, #ffffff) 65%);
  box-shadow:
...
.hub-satellite.tone-teal {
  border-color: var(--app-line, #5eead4);
  background: radial-gradient(circle at 50% 28%, var(--app-surface-muted, #f0fdfa) 0%, var(--app-surface, #ffffff) 65%);
  box-shadow:
...
.hub-satellite.tone-cyan {
  border-color: var(--app-line, #22d3ee);
  background: radial-gradient(circle at 50% 28%, var(--app-surface-muted, #ecfeff) 0%, var(--app-surface, #ffffff) 65%);
  box-shadow:
...
.tone-purple .hub-satellite-icon { background: var(--app-surface-muted, #f8fafc); color: var(--app-muted, #475569); }
.tone-green .hub-satellite-icon { background: var(--app-surface-muted, #ecfdf5); color: var(--app-text-green, #16a34a); }
.tone-blue .hub-satellite-icon { background: var(--app-surface-muted, #eff6ff); color: var(--app-text-blue, #2563eb); }
.tone-orange .hub-satellite-icon { background: var(--app-surface-muted, #fff7ed); color: var(--app-text-red, #c2410c); }
.tone-indigo .hub-satellite-icon { background: var(--app-surface-muted, #ecfeff); color: var(--app-text-blue, #0e7490); }
.tone-teal .hub-satellite-icon { background: var(--app-surface-muted, #f0fdfa); color: var(--app-text-green, #0f766e); }
.tone-cyan .hub-satellite-icon { background: var(--app-surface-muted, #ecfeff); color: var(--app-text-blue, #0e7490); }
...
.tone-purple .hub-satellite-pill { background: var(--app-surface-muted, #f8fafc); color: var(--app-muted, #475569); }
.tone-green .hub-satellite-pill { background: var(--app-surface-muted, #ecfdf5); color: var(--app-text-green, #15803d); }
.tone-blue .hub-satellite-pill { background: var(--app-surface-muted, #eff6ff); color: var(--app-text-blue, #1d4ed8); }
...
.hub-satellite-badge {
  ...
  align-items: center;
  justify-content: center;
  border: 2px solid var(--app-line, #fff);
...
.hub-preview.tone-purple { border-color: var(--app-line, #cbd5e1); }
.hub-preview.tone-green { border-color: var(--app-line, #bbf7d0); }
.hub-preview.tone-blue { border-color: var(--app-line, #bfdbfe); }
.hub-preview.tone-indigo { border-color: var(--app-line, #c7d2fe); }
.hub-preview.tone-teal { border-color: var(--app-line, #99f6e4); }
.hub-preview.tone-cyan { border-color: var(--app-line, #67e8f9); }
...
.hub-preview-link:hover { background: var(--app-surface-muted, #f8fafc); }
.hub-preview-dot {
  width: 6px;
  height: 6px;
  border-radius: 999px;
...
.tone-purple .hub-preview-dot { color: var(--app-muted, #475569); }
.tone-green .hub-preview-dot { color: var(--app-text-green, #16a34a); }
.tone-blue .hub-preview-dot { color: var(--app-text-blue, #2563eb); }
.hub-preview-count {
  margin-left: auto;
  min-width: 20px;
  height: 20px;
...
@media (max-width: 820px) {
  ...
  .tone-purple .hub-mobile-icon { background: var(--app-surface-muted, #f8fafc); color: var(--app-muted, #475569); }
  .tone-green .hub-mobile-icon { background: var(--app-surface-muted, #ecfdf5); color: var(--app-text-green, #16a34a); }
  .tone-blue .hub-mobile-icon { background: var(--app-surface-muted, #eff6ff); color: var(--app-text-blue, #2563eb); }
  .tone-orange .hub-mobile-icon { background: var(--app-surface-muted, #fff7ed); color: var(--app-text-red, #c2410c); }
  .tone-indigo .hub-mobile-icon { background: var(--app-surface-muted, #ecfeff); color: var(--app-text-blue, #0e7490); }
  .tone-teal .hub-mobile-icon { background: var(--app-surface-muted, #f0fdfa); color: var(--app-text-green, #0f766e); }
  .tone-cyan .hub-mobile-icon { background: var(--app-surface-muted, #ecfeff); color: var(--app-text-blue, #0e7490); }
...
.hub-satellite--soon,
.hub-mobile-card--soon {
  opacity: 0.72;
  cursor: default;
  pointer-events: none;
}
.hub-preview-link--soon {
  opacity: 0.7;
  cursor: default;
  pointer-events: none;
}
.po-hub {
  background:
    radial-gradient(circle at 18% 12%, rgba(99, 102, 241, 0.06), transparent 40%),
    radial-gradient(circle at 82% 18%, rgba(14, 165, 233, 0.05), transparent 35%),
    radial-gradient(circle at 50% 92%, rgba(244, 63, 94, 0.04), transparent 40%),
    var(--hub-bg);
}
</style>
