<template>
  <div class="fcc" :class="{ 'fcc-gate': !dashboard }">
    ...
    <section v-else-if="!session" class="fcc-welcome">
      ...
      <form @submit.prevent="unlock" class="fcc-login">
        ...
        <label>Your six-digit code<input v-model="pin" class="fcc-pin" type="password" inputmode="numeric" autocomplete="off" maxlength="6" pattern="[0-9]{6}" placeholder="••••••" required /></label>
        <label v-if="needsEmail">Account email<input v-model="email" type="email" autocomplete="email" required /></label>
    ...
    <template v-if="dashboard">
      ...
      <main class="fcc-main">
        ...
        <section v-if="tab==='Calendar'" class="family-card fcc-calendar-toolbar"><label>Show work schedule<select v-model="workMode"><option value="hidden">Hidden</option><option value="busy">Work — no details</option><option value="details">Work — with details</option></select></label><label>View from<input v-model="calendarDate" type="date" /></label><button @click="openEditor('status')">＋ Set a status</button></section>
...
const events=computed(()=>entriesOf('event').sort((a,b)=>new Date(a.start_at)-new Date(b.start_at)));
const calendarEvents=computed(()=>familyCalendarEntries(dashboard.value?.entries || []));
...
async function unlock(){await run(async()=>{await resolveTenant();if((organization.value||agencyId.value)&&!tenant.value)return;try{await http.post('/unlock',{agencyId:tenant.value?.id,passcode:pin.value,email:email.value||undefined});pin.value='';await loadSession();}catch(e){if(e.response?.status===409||e.response?.data?.error?.message?.includes('email'))needsEmail.value=true;throw e;}});}
...
async function loadDashboard(){if(!householdId.value)return;refreshing.value=true;try{dashboard.value=(await http.get(`/households/${householdId.value}`)).data;}finally{refreshing.value=false;}}
