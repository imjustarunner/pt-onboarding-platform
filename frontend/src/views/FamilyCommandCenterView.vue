    ...
    <div v-if="editor || detail || redeem" class="fcc-modal-backdrop" @click.self="closeModal" @keydown.esc="closeModal">
      <section class="fcc-modal" role="dialog" aria-modal="true" :aria-label="editor ? 'Add '+draft.kind : detail?.title || redeem?.title" tabindex="-1" ref="modal">
        ...
        <form v-if="editor" @submit.prevent="saveEntry"><span class="eyebrow">MAKE ROOM FOR WHAT MATTERS</span><h2>{{ draft.id ? 'Edit' : 'Add' }} {{ draft.kind==='event'?'a personal event':draft.kind }}</h2>
          <FamilyVoiceEvent v-if="draft.kind==='event' && !draft.id" :key="householdId" :http="http" :household-id="householdId" :timezone="dashboard.household.timezone" :initially-open="voiceOpen" @draft="applyVoiceDraft" @active="voiceActive=$event" />
          ...
          <label v-if="['chore','grocery','shopping','packing'].includes(draft.kind)">Category<input v-model="draft.metadata.category" placeholder="Produce, school, outdoors…" /></label><label>Notes<textarea v-model="draft.metadata.notes" /></label><button class="fcc-primary" :disabled="busy || voiceActive">{{ busy?'Saving…':'Save to our family' }}</button>
        </form>
...
const voiceOpen=ref(false),voiceActive=ref(false);
...
async function loadDashboard(){if(!householdId.value)return;const current=++dashboardRequest;refreshing.value=true;try{const id=householdId.value;const data=(await http.get(`/households/${id}`)).data;if(current!==dashboardRequest||id!==householdId.value||!session.value)return;data.entries=(data.entries || []).map(e=>e.kind==='event'?{...e,metadata:familyEventMetadata(e.title,e.metadata)}:e);dashboard.value=data;void loadGoogleHome(id,current);}finally{if(current===dashboardRequest)refreshing.value=false;}}
...
function openEditor(kind){voiceOpen.value=false;voiceActive.value=false;previousFocus=document.activeElement;draft.value={kind,title:kind==='status'?'Home':'',memberUserId:null,startAt:['event','status','chore','meal'].includes(kind)?localInput(new Date()):'',endAt:['event','status'].includes(kind)?localInput(new Date(Date.now()+3600000)):'',metadata:{eventType:'family',autoTheme:true,color:'#7976d7',points:kind==='reward'?50:10,approval:true,recurrence:'none',rotation:[],reminderMinutes:0}};editor.value=true;nextTick(()=>modal.value?.querySelector('input,select')?.focus());}
...
function selectEventTheme(type){Object.assign(draft.value.metadata,{eventType:type.id,color:type.color,autoTheme:false,artworkVariant:null,artwork:null});}
...
function closeModal(){voiceActive.value=false;voiceOpen.value=false;editor.value=false;detail.value=null;redeem.value=null;previousFocus?.focus?.();}
...
async function saveEntry(){if(voiceActive.value)return;await run(async()=>{
  ...
  try{await loadDashboard();}catch{notice.value='Event saved. Reconnect or refresh to load the rest of the calendar.';}
});}
...
.fcc-notice{background:var(--soft);color:var(--purple);bottom:auto;top:24px}
...
.fcc-modal-backdrop{position:fixed;inset:0;background:#28233755;backdrop-filter:blur(5px);z-index:10000;display:flex;align-items:center;justify-content:center;padding:24px}
.fcc-modal{position:relative;background:var(--surface);width:560px;max-width:100%;max-height:90dvh;overflow-y:auto;border-radius:20px;padding:35px;box-shadow:0 20px 100px #29203c33}
