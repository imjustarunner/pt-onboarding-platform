...
async function loadGoogleHome(id){try{const from=new Date(Date.now()-86400000).toISOString(),to=new Date(Date.now()+30*86400000).toISOString();const {data}=await http.get(`/households/${id}/calendar-view`,{params:{from,to,work:'hidden'}});if(id!==householdId.value||!session.value)return;googleHomeEvents.value=(data.events || []).filter(e=>e.source==='Google').map(e=>({...e,id:e.key,kind:'event',readOnly:true,start_at:e.start,end_at:e.end,metadata:familyEventMetadata(e.title,{...e.metadata,address:e.location,allDay:!!e.startDate})}));}catch(e){if(id===householdId.value)googleHomeEvents.value=[];}}
...
async function loadDashboard(){if(!householdId.value)return;refreshing.value=true;try{const id=householdId.value;const data=(await http.get(`/households/${id}`)).data;if(id!==householdId.value)return;data.entries=(data.entries || []).map(e=>e.kind==='event'?{...e,metadata:familyEventMetadata(e.title,e.metadata)}:e);dashboard.value=data;await loadGoogleHome(id);}finally{refreshing.value=false;}}
...
async function logout(){await run(async()=>{await http.post('/logout');session.value=null;dashboard.value=null;googleHomeEvents.value=[];await resolveTenant();});}
function localInput(date){return isoToZonedDatetimeLocal(date, dashboard.value.household.timezone);}
function openEditor(kind){previousFocus=document.activeElement;draft.value={kind,title:kind==='status'?'Home':'',memberUserId:null,startAt:['event','status','chore','meal'].includes(kind)?localInput(new Date()):'',endAt:['event','status'].includes(kind)?localInput(new Date(Date.now()+3600000)):'',metadata:{eventType:'family',autoTheme:true,color:'#7976d7',points:kind==='reward'?50:10,approval:true,recurrence:'none',rotation:[],reminderMinutes:0}};editor.value=true;nextTick(()=>modal.value?.querySelector('input,select')?.focus());}
