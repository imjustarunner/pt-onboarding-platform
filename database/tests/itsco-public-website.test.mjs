...
test('official names classify junior highs and exclude fake Other districts without blocking real new districts',()=>{
 const out=assembleSchoolDistricts([{id:1,name:'Short name',official_name:'Cheyenne Mountain Junior High School',district_name:'D12'},{id:2,name:'QR Self Serve School',district_name:'Other'},{id:3,name:'Unknown school',district_name:'Other'}],{},resolveCanonicalDistrict,()=>null);
 assert.equal(out.length,1);assert.equal(out[0].schools.length,1);assert.equal(out[0].schools[0].level,'Middle Schools');assert.equal(out[0].schools[0].name,'Cheyenne Mountain Junior High School');
});
