...
Notes:
  - This project’s migration runner skips statements that start with `--`, so we use block comments.
  - MySQL/InnoDB index key length limits (utf8mb4) require a prefix index for the ics_url uniqueness constraint.
*/
...
CREATE TABLE IF NOT EXISTS user_external_calendar_feeds (
  ...
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  /* NOTE: Use a prefix index to stay under InnoDB max key length with utf8mb4. */
  UNIQUE KEY uq_user_external_calendar_feeds_calendar_url (calendar_id, ics_url(512)),
...
SELECT u.id, ('Legacy EHR (ICS)' COLLATE utf8mb4_0900_ai_ci), TRUE, NULL
FROM users u
WHERE u.external_busy_ics_url IS NOT NULL
  ...
  AND NOT EXISTS (
    ...
    WHERE c.user_id = u.id AND c.label = ('Legacy EHR (ICS)' COLLATE utf8mb4_0900_ai_ci)
    LIMIT 1
  );
...
JOIN user_external_calendars c
  ON c.user_id = u.id AND c.label = ('Legacy EHR (ICS)' COLLATE utf8mb4_0900_ai_ci)
WHERE u.external_busy_ics_url IS NOT NULL
  AND TRIM(u.external_busy_ics_url) <> ''
  AND NOT EXISTS (
    SELECT 1
    FROM user_external_calendar_feeds f
    WHERE f.calendar_id = c.id AND f.ics_url = (u.external_busy_ics_url COLLATE utf8mb4_0900_ai_ci)
    LIMIT 1
  );
